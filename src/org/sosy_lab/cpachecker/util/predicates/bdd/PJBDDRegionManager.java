...
public class PJBDDRegionManager implements RegionManager {
  ...
  public PJBDDRegionManager(Configuration pConfig, LogManager pLogger)
    ...
    if (!buildFromConfig.getAllValidBDDTypes().contains(buildFromConfig.getUseDDType())) {
      logger.log(
          Level.INFO,
          "Could not find PJBDD BDD type "
              + buildFromConfig.getUseDDType()
    ...
    private void resolveProperties(CreatorBuilder pBuilder) {
      ...
      switch (tableType) {
        ...
        case "ConcurrentHashBucket":
          ...
          break;
        case "CASArray":
          pBuilder.setTableType(Builders.TableType.CASArray);
          break;
        case "Array":
