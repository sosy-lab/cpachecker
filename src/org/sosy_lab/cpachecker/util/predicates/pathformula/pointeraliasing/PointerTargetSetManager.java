...
class PointerTargetSetManager {
   ...
   * @param regMk Regions maker for the B&B memory model
   */
  ...
  private <I extends Formula> Formula makeDereference(
      final CType type, final I address, final SSAMapBuilder ssa, final String region) {
    String ufName = CToFormulaConverterWithPointerAliasing.getPointerAccessName(type);
  ...
  private static PersistentSortedMap<String, PersistentList<PointerTarget>> addToTarget(final String base,
    ...
    String ufName = CToFormulaConverterWithPointerAliasing.getPointerAccessName(checkIsSimplified(targetType));
    if (useBnB) {
      ufName = BnBRegionsMaker.getNewUfName(ufName, region);
    }
    PersistentList<PointerTarget> targetsForType =
        targets.getOrDefault(ufName,PersistentLinkedList.of());
