...
public final class DynamicMemoryHandler {
            ...
            if (!isAllocation) {
              if (CExpressionVisitorWithPointerAliasing.isRevealingType(lhsType)) {
                ...
                final Optional<PointerBase> lhsPointer =
                    lhs.accept(new PointerApproximatingVisitor(typeHandler, edge, pts));
  ...
  private void handleDeferredAllocationsInAssignment(
    ...
    final PointerApproximatingVisitor pointerApproximatingVisitor =
        new PointerApproximatingVisitor(typeHandler, edge, pts);
    ...
    if (lhs instanceof CIdExpression cIdExpression) {
      ...
      pts.removeDeferredAllocationPointer(
              PointerBase.forVariable(cIdExpression.getDeclaration(), pts.getCallStackDepth()))
