...
public class ExpressionToFormulaVisitor extends DefaultCExpressionVisitor<Formula, UnrecognizedCCodeException>
  ...
  public Formula visit(CFieldReference fExp) throws UnrecognizedCCodeException {
    ...
    if (fieldRef instanceof CIdExpression) {
      ...
      if (decl instanceof CDeclaration && ((CDeclaration)decl).isGlobal()) {
        ...
        return conv.makeVariable(JSToFormulaConverter.exprToVarNameUnscoped(fExp), fExp.getExpressionType(), ssa);
      }
    }
  ...
  public Formula visit(CFunctionCallExpression e) throws UnrecognizedCCodeException {
    ...
    if (functionNameExpression instanceof CIdExpression) {
      ...
      } else if (JSToFormulaConverter.UNSUPPORTED_FUNCTIONS.containsKey(functionName)) {
        throw new UnsupportedCCodeException(JSToFormulaConverter.UNSUPPORTED_FUNCTIONS.get(functionName), edge, e);
        ...
        if (parameters.size() == 1) {
          ...
          if (formulaType.isFloatingPointType()) {
            ...
            FloatingPointFormula zero =
                fpfmgr.makeNumber(0.0, (FloatingPointType) formulaType);
        ...
        if (parameters.size() == 2) {
          ...
          if (formulaType.isFloatingPointType()) {
            ...
            FloatingPointFormula param1 =
                (FloatingPointFormula) processOperand(parameters.get(1), paramType, paramType);
            FloatingPointFormula zero =
                fpfmgr.makeNumber(0, (FloatingPointType) formulaType);
      ...
      } else if (!JSToFormulaConverter.PURE_EXTERNAL_FUNCTIONS.contains(functionName)) {
      ...
      String escapedName = JSToFormulaConverter.exprToVarName(functionNameExpression, function);
