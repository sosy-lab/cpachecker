...
public class Mathsat5Factory {
  ...
  public static Mathsat5FormulaManager createFormulaManager(Configuration config, LogManager logger) throws InvalidConfigurationException {
    ...
    if (options.useBitwise) {
      if (options.useIntegers) {
        ...
        return new BitwiseMathsat5FormulaManager(config, logger, options.bitWidth);
      }

    } else {
      return new ArithmeticMathsat5FormulaManager(config, logger, options.useIntegers);
    }
  }
