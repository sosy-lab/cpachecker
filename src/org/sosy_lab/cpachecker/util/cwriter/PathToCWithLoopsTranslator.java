...
public class PathToCWithLoopsTranslator extends PathTranslator {
  ...
  private Pair<String, CFAEdge> recreateLoop(
    ...
    // handled edges to the handledEdges list, so they won't occur twice in the
    // generated c code
  ...
  /**
   * Creates a new label for a loop, thus, for the into loop case a label before the loop and for
  ...
  /**
   * This method processes a given edge which may be inside a loop. The suffix is used to jump to a
   * label if we have found an assume edge. If the suffix is empty this is an assume which leads to
   * program termination if not hold, in order to have only the one path of the program we want to
   * analyze afterward.
   *
