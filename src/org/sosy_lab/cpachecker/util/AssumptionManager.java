...
public abstract class AssumptionManager {

  final CBinaryExpressionBuilder cBinaryExpressionBuilder;
  final MachineModel machineModel;

  public AssumptionManager(MachineModel pMachineModel, LogManager pLogger) {
    ...
    for (CExpression assumption : from(assumptions).skip(1)) {
      result =
          cBinaryExpressionBuilder
              .buildBinaryExpression(assumption, result, BinaryOperator.BINARY_AND);
    }
    if (negate) {
      result =
          cBinaryExpressionBuilder
              .buildBinaryExpression(result, CIntegerLiteralExpression.ZERO, BinaryOperator.EQUALS);
    }
    return result;
  }

  public CExpression
      getResultOfOperation(CExpression operand1, CExpression operand2, BinaryOperator operator)
          throws UnrecognizedCodeException {
  ...
  public CExpression getAdditiveAssumption(
    ...
    CExpression term1 =
        cBinaryExpressionBuilder
            .buildBinaryExpression(operand2, CIntegerLiteralExpression.ZERO, term1Operator);
   ...
   *
   * <p>
   * The necessary assumption for division or modulo to be free from overflows looks as follows:
   *
   * <p>
  ...
  public void addLeftShiftAssumptions(
    ...
    // But this is (currently) not considered as overflow!
    /*
     * result.add( cBinaryExpressionBuilder.buildBinaryExpression(operand2, width,
