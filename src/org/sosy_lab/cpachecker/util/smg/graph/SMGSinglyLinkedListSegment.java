...
public class SMGSinglyLinkedListSegment extends SMGObject {
  ...
  public SMGObject join(SMGObject otherObj) {
    ...
    if (otherObj instanceof SMGSinglyLinkedListSegment otherSLL) {
      newMinLength = Integer.min(newMinLength, otherSLL.getMinLength());
      if (otherObj instanceof SMGDoublyLinkedListSegment otherDLL) {
        return otherDLL.join(this);
      }
    }
    return ((SMGSinglyLinkedListSegment) this.copyWithNewNestingLevel(newNestingLevel))
