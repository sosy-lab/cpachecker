...
/**
 * This Class build binary expression. It handles the promotion and conversion of C-types, depending
 * on the operands, as defined in ISO-C99.
 *
 * <p>This class should be used to get the correct CTypes for a binary operation. There are always 4
 * types in a binary operation, that may be different:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Example: <code>{@code "short l = (char)4 < (long)5;"}</code>
 *
 * <p>
 *
 * <ul>
 *   <li>The values 4 and 5 are used in the calculation as values of type 'char' and 'long', because
 *       they are casted to.
 *   <li>For the calculation the values are widened to 'long', because this is the 'longest type'
 *       (informally). Then the calculation is performed as 'long'.
 *   <li>The result has type 'int', because the operation is a relational operator (LESS_THAN). This
 *       needs a cast from 'long' to 'int'.
 * </ul>
 *
 * <p>As last step the result would be casted to 'short'. However, this last step is not part of the
 * binary operation, but only of the assignment, and this is not handled within this class.
 */
public class CBinaryExpressionBuilder {
  ...
  /**
   * This method returns a binaryExpression. It is better (safer) to use this method instead of the
   * Constructor of the CBinaryExpression, because the C-type of the expression is build from the
  ...
  public CBinaryExpression buildBinaryExpression(
      CExpression op1, CExpression op2, BinaryOperator op) throws UnrecognizedCodeException {
    ...
    // For calculation type determination, bit-field types are treated the same as their actual
    // types
  ...
  /**
   * This method does the same as {@link #buildBinaryExpression(CExpression, CExpression,
   * BinaryOperator)}, but does not throw a checked exception. Use this whenever the operation is
   * guaranteed to be valid.
   */
  public CBinaryExpression buildBinaryExpressionUnchecked(
      CExpression op1, CExpression op2, BinaryOperator op) {
    try {
  ...
  /**
   * Create an expression that is the negation of the input. This is better than just building "expr
   * == 0" because if "expr" contains a relational operator we can swap it instead. This makes it
   * easier for many CPAs to handle it.
   */
  public CBinaryExpression negateExpressionAndSimplify(final CExpression expr)
    ...
    // at this point, we have an expression, that is not directly boolean (!a, !(a+b), !123), so we
    // compare it with Zero.
    // ISO-C 6.5.3.3: Unary arithmetic operators: The expression !E is equivalent to (0==E).
    // TODO do not wrap numerals, replace them directly with the result? This may be done later with
   ...
   *
   * <p>This method calculates the type of the result of a binary operation. This type may be
   * different from the type of the calculation itself, in most cases it is equal, only sometimes it
   * is smaller. It should not be bigger than the type of the calculation. The result of the
   * calculation must be casted to the type, that this method returns.
   *
   ...
   * private method, only visible for Junit-test.
   *
   * <p>This method returns the "common real type" of a binary expression. For detail see
   * ISO-C-Standard:
   *
   * <ul>
  ...
  CType getCalculationTypeForBinaryOperation(
    ...
    if ((pType1 instanceof CPointerType
            || pType1 instanceof CFunctionType
            || pType1 instanceof CArrayType)
        && (pType2 instanceof CPointerType
            || pType2 instanceof CFunctionType
    ...
    if (pType1 instanceof CCompositeType
        || pType1 instanceof CElaboratedType
        || pType2 instanceof CCompositeType
        || pType2 instanceof CElaboratedType) {
      throw new UnrecognizedCodeException(
  ...
  /**
   * This method returns the (simplified) type of the second operand, if the first operand was from
   * CSimpleType. This method does not depend on the first type.
   *
  ...
  /**
   * This method should return the "common real type" of an arithmetic binary expression. For detail
   * see C-Standard: 6.3.1.8 Usual arithmetic conversions. This method should be better than the
   * (current) eclipse-parser, that does not always return the correct common type.
   *
   * <p>(Example: char + char --> int, not char. For chars there could be an error in the
   * calculation of 120*8/8.)
   *
