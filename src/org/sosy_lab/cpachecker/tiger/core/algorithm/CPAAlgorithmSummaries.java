...
import org.sosy_lab.cpachecker.tiger.testgen.summaries.SummaryCoverage;

import com.google.common.collect.Iterables;

@Options(prefix="cpa")
/**
 * Like {@link CPAAlgorithm}, but during exploration check for coverage of summaries of removed subgraphs.
 */
public class CPAAlgorithmSummaries implements Algorithm, StatisticsProvider {
  ...
  private final boolean mUseSummarise;
  private StopOperator stopOperator;
  private SummaryCoverage sumCov;

  public CPAAlgorithmSummaries(ConfigurableProgramAnalysis cpa, LogManager logger,
      Configuration config, ShutdownNotifier pShutdownNotifier,
      List<AbstractState> pSummaries, SummaryCoverage pSumCov, boolean pMUseSummarise) throws InvalidConfigurationException {
    config.inject(this);
    this.cpa = cpa;
    this.logger = logger;
    this.shutdownNotifier = pShutdownNotifier;
    this.stopOperator = cpa.getStopOperator();
    this.summaries = pSummaries;
    this.mUseSummarise = pMUseSummarise;
    this.sumCov = pSumCov;
  ...
  private boolean run0(final ReachedSet reachedSet) throws CPAException, InterruptedException {
    ...
    while (reachedSet.hasWaitingState()) {
      ...
      if (this.mUseSummarise){
        // check if the state is covered by a summary
        boolean res = sumCov.covers(state, precision, summaries);
