...
import org.sosy_lab.cpachecker.tiger.testcases.PreciseInputsTestCase;
import org.sosy_lab.cpachecker.tiger.testcases.TestCase;

public class AlternatingRefiner implements Refiner {

  private final PredicateCPARefiner mPredicateRefiner;
  ...
  private final FunctionEntryNode mEntryNode;
  ...
  public AlternatingRefiner(GuardedEdgeAutomatonCPA pCoverAutomatonCPA, GuardedEdgeAutomatonCPA pPassingAutomatonCPA, LocationCPA pLocationCPA, CallstackCPA pCallStackCPA, AssumeCPA pAssumeCPA, CFAPathCPA pCFAPathCPA, ARGCPA pARTCPA, FunctionEntryNode pEntryNode, CFANode pEndNode, PredicateCPARefiner pPredicateRefiner, Configuration pConfiguration, LogManager pLogManager) {
    ...
    mIntermediateTestSuite = new HashSet<>();
  }
  ...
  public boolean performRefinement(ReachedSet pReached) throws CPAException, InterruptedException {
    ...
    CounterexampleInfo lCounterexampleInfo = mPredicateRefiner.performRefinementWithInfo(pReached);

    if (lCounterexampleInfo.isSpurious()) {
    ...
    TestCase lTestCase = TestCase.fromCounterexample(lCounterexampleInfo.getTargetPathModel(), mLogManager);
    ...
    */
    ...
    try {
      lAlgorithm = new CPAAlgorithm(lCPA, mLogManager, mConfiguration, ShutdownNotifier.create());
    } catch (InvalidConfigurationException e1) {
      throw new RuntimeException(e1);
    }

    AbstractState lInitialElement = lCPA.getInitialState(mEntryNode);
    ...
    if (!lMissesInput && lEndNode != null) {
      if (((LocationState)lEndNode.get(0)).getLocationNode().equals(mEndNode)) {
    ...
    AbstractState lLastElement = pReached.getLastState();

    CompositePrecision lPrecision = (CompositePrecision)pReached.getPrecision(lLastElement);

    ARGState lLastARTElement = (ARGState)lLastElement;

    CompositeState lWrappedElement = (CompositeState)lLastARTElement.getWrappedState();
    ...
    Map<ARGState, Precision> lRemoveElements = new HashMap<>();

    for (ARGState lParent : lLastARTElement.getParents()) {
      ...
      List<Precision> lNewPrecisions = new ArrayList<>(lProductAutomatonPrecision.getPrecisions().size());
      ...
      List<Precision> lNewPrecisions2 = new ArrayList<>(lParentPrecision.getPrecisions().size());
      ...
      for (int lIndex = 2 + 1; lIndex < lWrappedElement.getNumberOfStates(); lIndex++) {
        lNewPrecisions2.add(lParentPrecision.get(lIndex));
      }
    ...
    ARGReachedSet lARTReached = new ARGReachedSet(pReached, mARTCPA);

    throw new UnsupportedOperationException("Implement!");

    /*
    for (Map.Entry<ARGState, Precision> lEntry : lRemoveElements.entrySet()) {
