...
public class RangeExecution extends NestingAlgorithm {
  ...
  private List<Path> computeAndLoadBound(CFANode mainFunction, ReachedSet pReached) {
    ...
    for (int i = 0; i < valuesForLoopBound.size(); i++) {
      ...
      try {
        Collection<AdditionalConfigParameter> additionalParams;

        additionalParams =
            Lists.newArrayList(
                new AdditionalConfigParameter(
  ...
  protected NestedAnalysis createNextAlgorithm(
      ...
      Collection<AdditionalConfigParameter> pAdditionalParams)
