...
public class PThenNoneOperator extends PartitioningBudgetOperator {

  @Option(secure=true, description="Add at most k% properties to one partition.")
  private int k = 5;

  public PThenNoneOperator(Configuration pConfig, LogManager pLogger)
  ...
  public Partitioning partition(
    ...
    if (pLastCheckedPartitioning.getStatus() == PartitioningStatus.P_FOR_EACH) {
      return create(PartitioningStatus.BREAK, InfinitePropertyBudgeting.INSTANCE,
          getPartitionBudgetingOperator(),
          ImmutableList.of());
    } else {
      return create(PartitioningStatus.P_FOR_EACH,
          getPropertyBudgetingOperator(),
          getPartitionBudgetingOperator(),
          partitionsWithAtMostPercentage(k, pToCheck, pPropertyExpenseComparator));
    }
  }
