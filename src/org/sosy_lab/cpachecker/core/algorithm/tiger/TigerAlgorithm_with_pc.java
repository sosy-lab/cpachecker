...
import org.sosy_lab.cpachecker.core.algorithm.tiger.util.TestCase_with_pc;
import org.sosy_lab.cpachecker.core.algorithm.tiger.util.TestSuite_with_pc;
...
public class TigerAlgorithm_with_pc implements Algorithm, PrecisionCallback<PredicatePrecision>, StatisticsProvider, Statistics {
  ...
  private TestSuite_with_pc testsuite;
  ...
  public TigerAlgorithm_with_pc(Algorithm pAlgorithm, ConfigurableProgramAnalysis pCpa, ShutdownNotifier pShutdownNotifier,
    ...
    testsuite = new TestSuite_with_pc(bddCpaNamedRegionManager);
  ...
  private boolean isCovered(int goalIndex, Goal_with_pc lGoal) {
    ...
    for (TestCase_with_pc testcase : testsuite.getTestCases()) {
      ...
      Region coveringRegion = testcase.getRegion();
      /* the critical edge of the goal might be covered by a different (less constrained) presence condition
       * However, I cannot use this code anymore because testcase.getPath now returns only a list of CFAEdges (without the ARGstates).
  ...
  private boolean testGeneration(LinkedList<Pair<ElementaryCoveragePattern,Region>> pTestGoalPatterns, Pair<Boolean, LinkedList<Edges>> pInfeasibilityPropagation) throws CPAException, InterruptedException {
    ...
    while (!pTestGoalPatterns.isEmpty()) {
      ...
      while (!stop && !remainingPCforGoalCoverage.isFalse()) {
        ...
        // update PC coverage todo
        if (testsuite.isKnownAsInfeasible(lGoal) &&
    ...
    if (testsuite.getTimedOutGoals().isEmpty()) {
      ...
      if (! timeoutStrategy.equals(TimeoutStrategy.RETRY_AFTER_TIMEOUT)) {
        ...
        do {
          ...
          Collection<Entry<Integer, Pair<Goal_with_pc,Region>>> set;
          ...
          for (Entry<Integer, Pair<Goal_with_pc,Region>> entry : set) {
            goalIndex = entry.getKey();
            Goal_with_pc lGoal = entry.getValue().getFirst();
  ...
  private ReachabilityAnalysisResult runReachabilityAnalysis(int goalIndex, Goal_with_pc pGoal, NondeterministicFiniteAutomaton<GuardedEdgeLabel> pPreviousGoalAutomaton, Pair<Boolean, LinkedList<Edges>> pInfeasibilityPropagation, Region remainingPCforGoalCoverage) throws CPAException, InterruptedException {
   ...
   * @return
   */
  private Goal_with_pc constructGoal(int pIndex, ElementaryCoveragePattern pGoalPattern, GuardedEdgeLabel pAlphaLabel,
