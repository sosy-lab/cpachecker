...
public class CounterexampleCheckAlgorithm
  ...
  @Option(
      ...
      name = "ambigiousARG",
      description =
          "True if the path to the error state can not always be uniquely determined from the"
              + " ARG.\n"
              + "This is the case e.g. for Slicing Abstractions, where the abstraction states in"
              + " the ARG\n"
              + "do not form a tree!")
    ...
    if (checkTime.getNumberOfIntervals() > 0) {
      out.println(
          "Number of infeasible paths:         "
              + numberOfInfeasiblePaths
              + " ("
              + toPercent(numberOfInfeasiblePaths, checkTime.getNumberOfIntervals())
              + ")");
