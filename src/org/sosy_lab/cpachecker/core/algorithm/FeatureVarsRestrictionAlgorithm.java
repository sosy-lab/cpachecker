...
import org.sosy_lab.cpachecker.cpa.featurevariables.FeatureVarsState;
...
public class FeatureVarsRestrictionAlgorithm implements Algorithm, StatisticsProvider, Statistics {
  ...
  public FeatureVarsRestrictionAlgorithm(Algorithm algorithm,
  ...
  public boolean run(ReachedSet reached) throws CPAException, InterruptedException {
        ...
        if (feasibility) {
          ...
          for (AbstractState x : ((CompositeState)errorState.getWrappedState()).getWrappedStates()) {
            if (x instanceof FeatureVarsState) {
              ...
              if (manager==null) {
                manager = ((FeatureVarsState)x).getManager();
    ...
    if (errorSummary!= null) {
      logger.log(Level.INFO, "ErrorSummary(" + errorsHit + "): " + manager.dumpRegion(errorSummary).replace("__SELECTED_FEATURE_", ""));
    } else {
      logger.log(Level.INFO, "ErrorSummary(0): No Errors");
    }
    return sound;
  }
