...
class KInductionProver implements AutoCloseable {
  ...
  */
      ...
      if (loops.getCount() > 1) {
        ...
        if (incomingEdges.size() > 1) {
          ...
          if (!isProgramConcurrent) {
            targetLocations = pTargetLocationProvider.tryGetAutomatonTargetLocations(cfa.getMainFunction(), false);
          }
        }
   ...
   */
  private BooleanFormula getCurrentLoopHeadInvariants() {
  ...
  private BooleanFormula getCurrentLocationInvariants(CFANode pLocation) {
    if (invariantGenerationRunning) {
      UnmodifiableReachedSet currentInvariantsReachedSet = getCurrentInvariantsReachedSet();
  ...
  public final boolean check() throws CPAException, InterruptedException {
    ...
    if (previousFormula != null && this.previousK == k) {
      ...
      safePredecessors = stepCaseBFMGR.not(createFormulaFor(predecessorTargetStates));
    }
    ...
    for (BooleanFormula knownLoopHeadInvariant : BMCAlgorithm.transform(knownLoopHeadInvariants, stepCaseFMGR, stepCasePFMGR)) {
      // Assert the invariant at all loop head states
      safePredecessors = stepCaseBFMGR.and(safePredecessors,
          stepCaseBFMGR.and(from(BMCHelper.assertAt(loopHeadStates, knownLoopHeadInvariant, stepCaseFMGR)).toList()));
    }
  ...
  private BooleanFormula createFormulaFor(Iterable<AbstractState> pStates) {

    BooleanFormula f = stepCaseBFMGR.makeBoolean(false);

    for (Map.Entry<CFANode, Collection<AbstractState>> entry : Multimaps.index(pStates, EXTRACT_LOCATION).asMap().entrySet()) {
      CFANode location = entry.getKey();
      BooleanFormula locationInvariants = getCurrentLocationInvariants(location);
      for (AbstractState targetState : entry.getValue()) {
        PredicateAbstractState pas = extractStateByType(targetState, PredicateAbstractState.class);
        PathFormula pathFormula = pas.getPathFormula();
        BooleanFormula instantiated = stepCaseFMGR.instantiate(locationInvariants, pathFormula.getSsa().withDefault(10));
        BooleanFormula formula = pathFormula.getFormula();
        formula = stepCaseBFMGR.and(formula, instantiated);
        f = stepCaseBFMGR.or(f, formula);
      }
    }
