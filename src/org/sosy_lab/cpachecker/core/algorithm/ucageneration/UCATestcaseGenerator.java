...
public class UCATestcaseGenerator  {
    ...
    if (!relevantStates.contains(lastState)) {
      lastStates =
          potentialLastStates.stream()
              .filter(s -> UCAGenerator.hasNoSuccessor(s, relevantStates))
   ...
   */
  private static int writeUCAForTestcase(
      Appendable sb,
      ...
      for (final ARGState child : getUncoveredChildrenView(s)) {
        ...
        if (edges.size() > 1) {
          ...
          if (pLastStates.contains(child)) {
            sb.append(String.format("GOTO %s", UCAGenerator.NAME_OF_NEWTESTINPUT_STATE));
          } else {
            AssumptionCollectorAlgorithm.finishTransition(
