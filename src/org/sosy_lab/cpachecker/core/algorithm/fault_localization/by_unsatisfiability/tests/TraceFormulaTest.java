...
public class TraceFormulaTest {
    ...
    enum LogKeys {
        TFTRACE, TFPRECONDITION, TFPOSTCONDITION;
    ...
    private TestResults runFaultLocalization(String name, String algorithm, Map<String, String> additionalOptions)
        throws Exception {
        final Configuration config = TestDataTools.configurationForTest()
            ...
            .forEach(
                line -> {
                  ...
                  if (result.size() == 2 && LogKeys.containsKey(result.get(0))) {
                      ...
                      String value = result.get(1).replace(" (TraceFormula.<init>, " + logLevel + ")", "");
                      if (keywords.contains(key)) {
                        switch (key) {
                            ...
                            case TFPOSTCONDITION:
                                ...
                                break;
                            default: throw new AssertionError("Unknown key: " + key);
                        }
                      }
                  }
                });
    ...
    private void test0(String program, String algorithm, Map<String, String> options, ExpectMap<LogKeys, Object> expected)
        throws Exception {
    ...
    private void assertNodesEquivalent(FormulaNode node1, FormulaNode node2) {
        //equivalent is not the same as equal since "a and b" is equivalent to "b and a" but not equal (Object#equal)
    ...
    test0(
        ...
        new ExpectMap<LogKeys, Object>()
            .expect(LogKeys.TFPRECONDITION, BooleanFormulaParser.parse("(`=_<BitVec, 32, >` __VERIFIER_nondet_int!2@ 4_32)"))
            .expect(LogKeys.TFPOSTCONDITION, BooleanFormulaParser.parse("(`=_<BitVec, 32, >` main::copyForCheck@2 main::test@3)")));
    }
    ...
    test0(
        ...
        "ERRINV",
        Map.of(),
        new ExpectMap<LogKeys, Object>().expect(
            LogKeys.TFTRACE,
    ...
    test0(
        "unit_test.c",
        "ERRINV",
        Map.of("faultLocalization.by_traceformula.errorInvariants.disableFSTF", "true"),
        new ExpectMap<LogKeys, Object>().expect(
            LogKeys.TFTRACE,
            BooleanFormulaParser.parse(
