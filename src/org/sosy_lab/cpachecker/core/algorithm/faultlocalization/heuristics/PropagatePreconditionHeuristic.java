...
public class PropagatePreconditionHeuristic<I extends FaultLocalizationOutput> implements
                                                                               FaultLocalizationHeuristic<I> {

  private final TraceFormula formula;

  public PropagatePreconditionHeuristic(TraceFormula pTraceFormula){
    formula = pTraceFormula;
  }

  @Override
  public Map<I, Integer> rank(ErrorIndicatorSet<I> result) {
    Set<I> all = FaultLocalizationHeuristicUtils.condenseErrorIndicatorSet(result);
    Map<I, Integer> rankMap = new HashMap<>();
    all.forEach(l -> {
      l.addReason(FaultLocalizationReason.hint("Pre Condition: " + ExpressionConverter.convert(formula.getPreCondition())));
      rankMap.put(l, 1);
    });
    return rankMap;
  }
