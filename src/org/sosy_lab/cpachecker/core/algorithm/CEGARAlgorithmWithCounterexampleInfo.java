...
public class CEGARAlgorithmWithCounterexampleInfo implements Algorithm, StatisticsProvider {
  ...
  private AbstractARGBasedRefiner createInstance(ConfigurableProgramAnalysis pCpa) throws CPAException, InvalidConfigurationException {
    ...
    if ((refinerObj == null) || !(refinerObj instanceof AbstractARGBasedRefiner)) {
      throw new InvalidComponentException(refiner, "Refiner", "Factory method did not return a AbstractARGBasedRefiner instance.");
    }

    return (AbstractARGBasedRefiner)refinerObj;
  }

  public CEGARAlgorithmWithCounterexampleInfo(Algorithm algorithm, ConfigurableProgramAnalysis pCpa, Configuration config, LogManager logger) throws InvalidConfigurationException, CPAException {
    config.inject(this);
   ...
   * @throws CPAException
   */
  public CEGARAlgorithmWithCounterexampleInfo(Algorithm algorithm, AbstractARGBasedRefiner pRefiner, Configuration config, LogManager logger) throws InvalidConfigurationException, CPAException {
    config.inject(this);
    this.algorithm = algorithm;
    this.logger = logger;
    ...
    try {
      ...
      do {
        ...
        if (refinementNecessary(reached)) {
          ...
          lCounterexampleInfo = refine(reached);

          refinementSuccessful = lCounterexampleInfo.isSpurious();
  ...
  private CounterexampleInfo refine(ReachedSet reached) throws CPAException, InterruptedException {
    ...
    try {
      lCounterexampleInfo = mRefiner.performRefinementWithInfo(reached);

      refinementResult = lCounterexampleInfo.isSpurious();
