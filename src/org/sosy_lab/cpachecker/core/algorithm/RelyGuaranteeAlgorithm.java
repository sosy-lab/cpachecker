...
public class RelyGuaranteeAlgorithm implements ConcurrentAlgorithm, StatisticsProvider{
   ...
   * Stores information about environmental transitions
   */
  public static class RelyGuaranteeEnvironment {
    ...
    public RelyGuaranteeEnvironment(int threadNo){
    ...
    private void printUnprocessedTransitions() {
      System.out.println();
  ...
  public RelyGuaranteeAlgorithm(CFA[] pCfas, CFAFunctionDefinitionNode[] pMainFunctions, ConfigurableProgramAnalysis[] pCpas, Configuration config, LogManager logger) {
    ...
    environment = new RelyGuaranteeEnvironment(threadNo);
  ...
  private void processEnvTransitions(int i) {
    ...
    Multimap<ARTElement, RelyGuaranteeEnvironmentalTransition> newEnvTransitions =  environment.getUnprocessedTransitions();
    ...
    while (!rgEdges.isEmpty()){
      ...
      for (int j=0; j<this.threadNo; j++){
        if (j!=i) {
          environment.addNewEnvEgde(j, new RelyGuaranteeCFAEdge(edge));
        }
      }
    }
  }
  ...
  private void syntacticCoverageCheck(int i) {
    ...
    for (ARTElement key : newEnvTransitions.keys()){
      for (RelyGuaranteeEnvironmentalTransition  et: newEnvTransitions.get(key)){
        ...
        if (f.isFalse() || pf.getFormula().isFalse() || isLocalAssigment(localEdge)) {
          ...
          for (RelyGuaranteeEnvironmentalTransition oldEt : environment.getEnvTransitionsCreatedBy(i)) {
            if (et.equals(oldEt)) {
              toDelete.add(Pair.of(key, et));
    ...
    environment.remberEnvTransitions(i,newEnvTransitions.values());
  ...
  private void semanticCoverageCheck(Vector<RelyGuaranteeCFAEdge> rgEdges, int i) {
   ...
   while(! toProcess.isEmpty()){
     ...
     List<RelyGuaranteeCFAEdge> envEdgesForThread = environment.getAppliedEnvEdgesForThread(i);
