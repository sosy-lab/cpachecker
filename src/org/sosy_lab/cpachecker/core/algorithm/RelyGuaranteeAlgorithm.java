...
public class RelyGuaranteeAlgorithm implements ConcurrentAlgorithm, StatisticsProvider{
  ...
  private Vector<RelyGuaranteeEnvEdge> newEnvTransitions;
  private Vector<RelyGuaranteeEnvEdge>[] envTransitionsForThread;
  private Vector<RelyGuaranteeEnvEdge>[] envTransitionsCreatedBy;
  // CPA for each thread
  ...
  public RelyGuaranteeAlgorithm(CFA[] pCfas, CFAFunctionDefinitionNode[] pMainFunctions, ConfigurableProgramAnalysis[] pCpas, LogManager logger) {
    ...
    newEnvTransitions = new Vector<RelyGuaranteeEnvEdge>();


    // create RelyGuaranteeThreadCPAAlgorithms for each thread
    // initialize envTransitionsForThread
    for (int i=0; i< this.threadNo; i++){
      threadCPA[i] = new RelyGuaranteeThreadCPAAlgorithm(cpas[i],newEnvTransitions,logger, globalVariables, i);
      envTransitionsForThread[i] = new Vector<RelyGuaranteeEnvEdge>();
      envTransitionsCreatedBy[i] = new Vector<RelyGuaranteeEnvEdge>();
    }
  ...
  private boolean isCovered(RelyGuaranteeEnvEdge env1, RelyGuaranteeEnvEdge env2) {
    ...
    /*Region region1 = env1.getAbstractionFormula().asRegion();
    Region region2 = env2.getAbstractionFormula().asRegion();
    if (!rManager.entails(region1, region2){
      return false;
    }*/
  ...
  private void addEnvTransitionsToCFA(int i) {
    ...
    for (RelyGuaranteeEnvEdge envTransition : this.envTransitionsForThread[i]){
  ...
  private void printEnvTransitions() {
    ...
    for (RelyGuaranteeEnvEdge edge : this.newEnvTransitions){
