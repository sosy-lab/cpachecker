...
public class RelyGuaranteeAlgorithm implements ConcurrentAlgorithm, StatisticsProvider{
  ...
  private Multimap<ARTElement, RelyGuaranteeEnvironmentalTransition> newEnvTransitions;
  ...
  /*public static class RelyGuaranteeEnvironment {
    ...
    public RelyGuaranteeEnvironment
  ...
  public RelyGuaranteeAlgorithm(CFA[] pCfas, CFAFunctionDefinitionNode[] pMainFunctions, ConfigurableProgramAnalysis[] pCpas, Configuration config, LogManager logger) {
    ...
    newEnvTransitions = HashMultimap.<ARTElement, RelyGuaranteeEnvironmentalTransition>create();
    envTransitionsCreatedBy = new Vector[threadNo];
    envEdgesForThread = new Vector[threadNo];
    newEnvEgesForThread = new Vector[threadNo];
  ...
  private void syntacticCoverageCheck(int i) {
    ...
    envTransitionsCreatedBy[i].addAll(newEnvTransitions.values());
  }
  ...
  private int pickThread() {
    int i=0;
    while(i<this.threadNo && newEnvEgesForThread[i].isEmpty() && hasBeenRun[i]){
      i++;
    }
    if (i==this.threadNo){
      return -1;
  ...
  private void printEnvTransitions() {
    ...
    for (ARTElement key : newEnvTransitions.keys()){
      for (RelyGuaranteeEnvironmentalTransition edge : newEnvTransitions.get(key)){
