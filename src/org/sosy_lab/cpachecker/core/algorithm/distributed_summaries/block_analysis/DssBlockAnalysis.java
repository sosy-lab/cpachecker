...
public class DssBlockAnalysis {
  ...
  private record AnalysisComponents(Algorithm algorithm, ConfigurableProgramAnalysis cpa,
      ReachedSet reached) {
  }

  private record AnalysisResult(boolean isSound, Collection<StateAndPrecision> summaries,
  ...
  private final DssStateSerializer serializer;
  ...
  public DssBlockAnalysis(
    ...
    serializer = new DssStateSerializer(dcpa);
  ...
  private List<AbstractState>
      computeViolationConditionStatesFromOrigin(Collection<@NonNull ARGState> state)
  ...
  private List<AbstractState> computeViolationConditionStatesFromBlockEnd(
      ...
      Collection<@NonNull ARGState> conditions)
      throws CPATransferException, SolverException, InterruptedException {
    ImmutableList.Builder<AbstractState> relevantViolations = ImmutableList.builder();
  ...
  private List<AbstractState>
      computeViolationConditionStates(Collection<@NonNull ARGState> violations)
          throws CPATransferException, SolverException, InterruptedException {
    ImmutableList.Builder<AbstractState> relevantViolations = ImmutableList.builder();
    for (ARGState violation : violations) {
  ...
  private ImmutableList<@NonNull StateAndPrecision>
      deduplicateStates(Collection<@NonNull StateAndPrecision> summaries)
  ...
  private Collection<DssMessage>
      reportPostconditions(Collection<@NonNull StateAndPrecision> summaries, boolean isSound)
    ...
    ImmutableMap<String, String> serialized = serializer.serialize(uniqueSummaries);
    messages.add(
        messageFactory.createDssPostConditionMessage(
            block.getId(),
            true,
  ...
  private Collection<DssMessage>
      reportViolationConditions(Collection<AbstractState> relevantViolations, boolean first)
    ...
    ImmutableMap<String, String> serialized = serializer.serialize(vcs.build());
    return ImmutableSet.of(
  ...
  public Collection<DssMessage> runInitialAnalysis()
    ...
    if (result.getAllViolations().isEmpty()) {
      ...
      for (AbstractState finalState : result.getFinalLocationStates()) {
        summariesWithPrecision
            .add(new StateAndPrecision(finalState, reachedSet.getPrecision(finalState)));
      }
  ...
  public DssMessageProcessing storePrecondition(DssPostConditionMessage pReceived)
    ...
    ImmutableList<@NonNull StateAndPrecision> deserializedStatesAndPrecisions =
        serializer.deserialize(pReceived);
    ...
    for (StateAndPrecision stateAndPrecision : deserializedStatesAndPrecisions) {
      processing =
          processing
              .merge(dcpa.getProceedOperator().processForward(stateAndPrecision.state()), true);
    }
    ...
    for (StateAndPrecision deserializedStateAndPrecision : deserializedStatesAndPrecisions) {
      for (StateAndPrecision stateAndPrecision : ImmutableSet
          .copyOf(preconditions.get(pReceived.getSenderId()))) {
   ...
   * @throws SolverException      thrown if solver runs into an error
   */
  public DssMessageProcessing
      storeViolationCondition(DssViolationConditionMessage pNewViolationCondition)
    ...
    ImmutableList<StateAndPrecision> deserializedStates =
        serializer.deserialize(pNewViolationCondition);
   ...
   * from the violation conditions stored via
   * {@link #storeViolationCondition(DssViolationConditionMessage)}
   *
    ...
    if (!result.summaries().isEmpty()) {
      messages.addAll(
          reportPostconditions(
              result.summaries(),
              result.isSound() && result.violationConditions().isEmpty()));
    }
    if (!result.violationConditions().isEmpty()) {
      messages.addAll(reportViolationConditions(result.violationConditions(), false));
    }
  ...
  private AnalysisResult analyzeViolationCondition(List<ARGState> violations, String id)
    ...
    for (StateAndPrecision stateAndPrecision : startStates.build()) {
      ...
      reachedSet.clear();
      reachedSet
          .add(stateAndPrecision.state(), maybePrecision.orElse(stateAndPrecision.precision()));
      reachedSet.forEach(
          abstractState -> Objects
              .requireNonNull(AbstractStates.extractStateByType(abstractState, BlockState.class))
      ...
      if (block.isAbstractionPossible()) {
        if (!result.getSummaries().isEmpty() && result.getAllViolations().isEmpty()) {
          ...
          for (AbstractState summary : result.getSummaries()) {
            summaryWithPrecision
                .add(new StateAndPrecision(summary, reachedSet.getPrecision(summary)));
          }
  ...
  private AbstractState makeStartState() throws InterruptedException {
    return dcpa
        .getInitialState(block.getInitialLocation(), StateSpacePartition.getDefaultPartition());
  }

  private Precision makeStartPrecision() throws InterruptedException {
    return dcpa
        .getInitialPrecision(block.getInitialLocation(), StateSpacePartition.getDefaultPartition());
  }
    ...
    for (Entry<String, StateAndPrecision> entry : ImmutableList.copyOf(preconditions.entries())) {
      ...
      preconditions.put(
          ...
          new StateAndPrecision(
              dcpa.reset(entry.getValue().state()),
