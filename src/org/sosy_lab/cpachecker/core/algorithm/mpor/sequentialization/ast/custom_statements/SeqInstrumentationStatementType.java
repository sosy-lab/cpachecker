...
public enum SeqInstrumentationStatementType {
   ...
   * {@link SeqInstrumentationStatement}s update e.g. ghost variables that are utilized by other
   * threads. But if no context-switch occurs due to the {@code goto}, then the ghost variable
   * updates are unnecessary and can be pruned.
   */
   ...
   * least one {@link SeqInstrumentationStatement} contains an empty bit vector evaluation
   * expression.
   */
  public final boolean isPrunedWithEmptyBitVectorEvaluation;

  SeqInstrumentationStatementType(
