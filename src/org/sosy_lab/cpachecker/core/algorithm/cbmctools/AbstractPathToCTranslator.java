...
public class AbstractPathToCTranslator {
  ...
  private List<StringBuffer> translatePath(final ARTElement firstElement,
    ...
    assert(firstElement.getLocalChildren().size() == 1);
    ARTElement firstElementsChild = (ARTElement)firstElement.getLocalChildren().toArray()[0];
    ...
    while(waitlist.size() > 0){
      ...
      int sizeOfChildsParents = childElement.getLocalParents().size();
      ...
      int sizeOfChildsChilds = childElement.getLocalChildren().size();
      ...
      if(sizeOfChildsChilds == 1){
        relevantChildrenOfElement.addAll(childElement.getLocalChildren());
      }
      // else find out whether children are on the path to error
      else{
        for(ARTElement child: childElement.getLocalChildren()){
