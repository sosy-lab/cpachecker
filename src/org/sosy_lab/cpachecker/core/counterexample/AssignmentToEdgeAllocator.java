...
public class AssignmentToEdgeAllocator {
  ...
  private String createComment(@SuppressWarnings("unused") CFAEdge pCfaEdge) {
    if (cfaEdge.getEdgeType() == CFAEdgeType.AssumeEdge) {
      ...
      return handleDclComment((ADeclarationEdge) cfaEdge);
        ...
        .getEdgeType() == CFAEdgeType.ReturnStatementEdge) { return handleReturnStatementComment(
            (AReturnStatementEdge) cfaEdge); }
  ...
  private String handleReturnStatementComment(AReturnStatementEdge pCfaEdge) {
    ...
    if (pCfaEdge.getExpression().isPresent()
        && pCfaEdge.getExpression().get() instanceof CExpression) {
      CExpression returnExp = (CExpression) pCfaEdge.getExpression().get();
  ...
  private String handleDclComment(ADeclarationEdge pCfaEdge) {
    ...
    if (pCfaEdge instanceof CDeclarationEdge) { return addressOfDcl(
        (CSimpleDeclaration) pCfaEdge.getDeclaration(), pCfaEdge); }
  ...
  private List<AAssignment> createAssignmentsAtEdge(CFAEdge pCFAEdge) {
    ...
    } else if (cfaEdge.getEdgeType() == CFAEdgeType.MultiEdge) { throw new AssertionError(
        "Multi-edges should be resolved by this point."); }
  ...
  private String handleAssume(AssumeEdge pCfaEdge) {
    ...
    if (pCfaEdge instanceof CAssumeEdge) { return handleAssume((CAssumeEdge) pCfaEdge); }
  ...
  private List<AAssignment> handleFunctionCall(FunctionCallEdge pFunctionCallEdge) {
    ...
    if (!(pFunctionCallEdge instanceof CFunctionCallEdge)) { return Collections.emptyList(); }
    ...
    for (CParameterDeclaration dcl : dcls) {
      assignments
          .addAll(handleDeclaration(dcl, pFunctionCallEdge.getSuccessor().getFunctionName()));
    }
  ...
  private List<AAssignment> handleAssignment(CLeftHandSide leftHandSide) {
    ...
    if (leftHandSide.getExpressionType()
        .getCanonicalType() instanceof CArrayType) { return Collections.emptyList(); }
  ...
  private List<AAssignment> handleDeclaration(ASimpleDeclaration dcl, String pFunctionName) {
    ...
    if (dcl instanceof CSimpleDeclaration) {
      ...
      // This whole interface is based on assignments.
      // For now, just return no assignments.
      if (isStructOrUnionType(dclType)) { return Collections.emptyList(); }
      ...
      // but it is illegal C code.
      // For now, just return no assignments
      if (dclType.getCanonicalType() instanceof CArrayType) { return Collections.emptyList(); }
  ...
  private List<AAssignment> handleSimpleValueLiteralsAssignments(ValueLiterals pValueLiterals,
  ...
  private class LModelValueVisitor implements CLeftHandSideVisitor<Object, RuntimeException> {
    ...
    private final Number evaluateNumericalValue(CExpression exp) {
      ...
      if (addressV.isUnknown() && !addressV.isNumericValue()) { return null; }
    ...
    public Object visit(CArraySubscriptExpression pIastArraySubscriptExpression) {
      ...
      if (type instanceof CArrayType
          || isStructOrUnionType(type)) { return valueAddress.getAddressValue(); }
    ...
    public Object visit(CFieldReference pIastFieldReference) {
      ...
      if (address == null) { return lookupReference(pIastFieldReference); }

      CType type = pIastFieldReference.getExpressionType().getCanonicalType();

      /*The evaluation of an array or a struct is its address*/
      if (type instanceof CArrayType
          || isStructOrUnionType(type)) { return address.getAddressValue(); }

      Object value = modelAtEdge.getValueFromMemory(pIastFieldReference, address);

      if (value == null) { return lookupReference(pIastFieldReference); }
    ...
    public Object visit(CIdExpression pCIdExpression) {
      ...
      if (address == null) { return lookupVariable(dcl); }
      ...
      if (value == null) { return lookupVariable(dcl); }
    ...
    private class AddressValueVisitor implements CLeftHandSideVisitor<Address, RuntimeException> {
      ...
      public Address visit(CArraySubscriptExpression pIastArraySubscriptExpression) {
        ...
        if (subscriptValueNumber == null) { return null; }
      ...
      public Address visit(CFieldReference pIastFieldReference) {
        ...
        if (fieldOwnerAddress == null) { return lookupReferenceAddress(pIastFieldReference); }

        BigDecimal fieldOffset = getFieldOffset(pIastFieldReference);

        if (fieldOffset == null) { return lookupReferenceAddress(pIastFieldReference); }

        Address address = fieldOwnerAddress.addOffset(fieldOffset);

        if (address == null) { return lookupReferenceAddress(pIastFieldReference); }
    ...
    private class ModelExpressionValueVisitor extends AbstractExpressionValueVisitor {
        ...
        switch (binaryOperator) {
          ...
          case MINUS: {
            ...
            if (addressValueV.isUnknown() || offsetValueV.isUnknown()
                ...
                || !offsetValueV.isNumericValue()) { return Value.UnknownValue
                    .getInstance(); }
      ...
      private Value handleAmper(CExpression pOperand) {
        if (pOperand instanceof CLeftHandSide) {
          ...
          if (address != null) { return new NumericValue(address.getAddressValue()); }
        } else if (pOperand instanceof CCastExpression) { return handleAmper(
            ((CCastExpression) pOperand).getOperand()); }
      ...
      protected Value evaluateCPointerExpression(CPointerExpression pCPointerExpression)
        ...
        if (value == null
            || !(value instanceof Number)) { return Value.UnknownValue.getInstance(); }
  ...
  private class ValueLiteralsVisitor extends DefaultCTypeVisitor<ValueLiterals, RuntimeException> {
    ...
    public ValueLiterals visit(CPointerType pointerType) throws RuntimeException {
      ...
      if (address == null) { return createUnknownValueLiterals(); }
    ...
    public ValueLiterals visit(CElaboratedType pT) throws RuntimeException {
      ...
      if (realType != null) { return realType.accept(this); }
    ...
    public ValueLiterals visit(CCompositeType compType) throws RuntimeException {
      ...
      if (compType.getKind() == ComplexTypeKind.ENUM) { return createUnknownValueLiterals(); }
     ...
     */
    private ValueLiteral handlePotentialIntegerOverflow(BigInteger pIntegerValue,
        CSimpleType pType) {
    ...
    private class ValueLiteralVisitor extends DefaultCTypeVisitor<Void, RuntimeException> {
      ...
      private void handleMemberField(CCompositeTypeMemberDeclaration pType, Address fieldAddress,
        ...
        if (visited.contains(visits)) { return; }
      ...
      public Void visit(CPointerType pointerType) throws RuntimeException {
        ...
        if (isStructOrUnionType(expectedType) || expectedType instanceof CArrayType) {
          ...
          ValueLiteralVisitor v =
              new ValueLiteralVisitor(address, valueLiterals, pointerExp, visited);
          expectedType.accept(v);
          return null;
        }
        ...
        if (valueAddress != null) {
          ...
          if (visited.contains(visits)) { return null; }
    ...
    private class ValueLiteralStructResolver extends DefaultCTypeVisitor<Void, RuntimeException> {
      ...
      public ValueLiteralStructResolver(ValueLiterals pValueLiterals, String pFunctionName,
      ...
      public Void visit(CCompositeType compType) throws RuntimeException {
        ...
        if (compType.getKind() == ComplexTypeKind.ENUM) { return null; }
      ...
      private void addStructSubexpression(Object pFieldValue, CFieldReference reference) {
        ...
        if (valueLiteral.isUnknown()) { return; }
