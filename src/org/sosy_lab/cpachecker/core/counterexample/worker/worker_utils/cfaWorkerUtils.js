...
function addEventsToCfa() {
  ...
  d3.selectAll(".cfa-split-edge")
    .on("mouseover", function(d) {
      ...
      const selection = d3.select("#cfa-edge_" + edgeSourceTarget.split("-")[0] + "-" + edgeSourceTarget.split("-")[1]);
...
const cfaWorkerCallback = (result) => {
  if (result.graph !== undefined) {
    ...
    if (!d3.select("#cfa-graph-" + result.id).empty()) {
      ...
      }).then(result => cfaWorkerCallback(result), error => cfaWorkerErrorCallback(error));
      return;
    }
    const id = result.id;
    ...
    const svg = d3.select("#cfa-graph-" + id).append("svg").attr("id", "cfa-svg-" + id).attr("class", "cfa-svg " + "cfa-svg-" + result.func);
