...
public class CallstackPccCPA extends AbstractCPA implements ConfigurableProgramAnalysisWithABM, ProofChecker {

  private final Reducer reducer = new CallstackReducer();

  public static CPAFactory factory() {
    return AutomaticCPAFactory.forType(CallstackPccCPA.class);
  }

  public CallstackPccCPA(Configuration config) throws InvalidConfigurationException {
    super("sep", "sep", new CallstackPccTransferRelation(config));
  }
  ...
  public boolean areAbstractSuccessors(AbstractState pElement, CFAEdge pCfaEdge,
    ...
    for (AbstractState e1 : pSuccessors) {
      ...
      for (AbstractState e2 : computedSuccessors) {
        if (((CallstackState) e1).equals(e2)) {
          found = true;
          break;
        }
      }
