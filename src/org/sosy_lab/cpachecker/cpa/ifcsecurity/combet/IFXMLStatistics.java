...
public class IFXMLStatistics implements Statistics {

  @Option(secure = true, description = "target file to hold the exported precision")
  @FileOption(FileOption.Type.OUTPUT_FILE)
  private Path xmlFile = Paths.get("C:/Users/mtoews/workspace/CPA/CPAchecker/ifc.xml");
  private Path xsdFile = Paths.get("C:/Users/mtoews/workspace/CPA/CPAchecker/xsd/case.xsd");
  private Path xsdextension = Paths.get("C:/Users/mtoews/workspace/CPA/CPAchecker/xsd/case_extensions.xsd");
  ...
  public IFXMLStatistics(DependencyTrackerCPA pCpa, Configuration pConfig, CFA pCfa,
      DependencyPrecision pPrecision) throws InvalidConfigurationException {
    this.cpa = pCpa;
    this.cfa = pCfa;
    this.precision = pPrecision;
    this.config = pConfig;
    pConfig.inject(this, IFXMLStatistics.class);
  }
  ...
  public void printStatistics(PrintStream pOut, Result pResult, UnmodifiableReachedSet pReached) {
    ...
    //Analysis
    if (xmlFile != null) {
      XSDManager<Case> manager = new XSDManager<>(xsdFile.toFile(), xsdextension.toFile());
      Case tree = buildXMLTree(pReached);
      manager.writeXML(tree, xmlFile.toFile());
    }

  }

  public Case buildXMLTree(UnmodifiableReachedSet pReached) {
    ...
    /* Case->Info->Project->Identifier */
    List<Path> fileName = cfa.getFileNames();
    assert (fileName.size() == 0);
    project.setIdentifier(fileName.get(0).getFileName().toString());

    /* Case->Info->Project->Resources */
    Resources resources = new Resources();
    project.setResources(resources);

    /* Case->Info->Project->Resources->Resource */
    /* File */
    for (int i = 0; i < fileName.size(); i++) {
      ...
      /* TODO relative path */
      resource.setURI(fileName.get(0).toString());
    }
    ...
    /* Case->Info->Tool->Name */
    tool.setName("CPAchecker");
    /* Case->Info->Tool->Version*/
    /* TODO flexible */
    tool.setVersion("25142");
    /* Case->Info->Tool->Configuration  */
    /* TODO relative path */
    ConfigurationExtension configuration = new ConfigurationExtension();
    configuration.setDescription(config.asPropertiesString());
    tool.setConfiguration(configuration);

    /* Case->Info->Analysis */
    Analysis analysis = new Analysis();
    info.setAnalysis(analysis);

    /* Case->Info->Analysis->Name */
    analysis.setName("Information Flow Analysis");
    /* Case->Info->Analysis->Attributes */
    Attributes attributes = new Attributes();
    analysis.setAttributes(attributes);

    /* Case->Info->Analysis->Attributes */
    AttributeExtension attribute = new AttributeExtension();
    attributes.getAttribute().add(attribute);

    /* Case->Info->Analysis->Attributes->Attribute->Policy */
    PolicyAlgebra<SecurityClasses> polAlg = new PolicyAlgebra<>();
    ...
    /* Case->Info->Analysis->Attributes->Attribute->Policy->Description */
    policy.setDescription(pol.toString());

    /* Case->Info->Analysis->Attributes->Attribute->Policy->Securityclasses */
    Securityclasses securityclasses = new Securityclasses();
    policy.setSecurityclasses(securityclasses);

    /* Case->Info->Analysis->Attributes->Attribute->Policy->Securityclasses->Securityclass */
    SortedSet<SecurityClasses> domain = polAlg.getDomain(pol);

    for (SecurityClasses seclass : domain) {
      securityclasses.getSecurityclass().add(seclass.toString());
    }

    /* Case->Info->Analysis->Attributes->Attribute->Policy->Allowed */
    Allowed allowed = new Allowed();
    policy.setAllowed(allowed);

    /* Case->Info->Analysis->Attributes->Attribute->Policy->Allowed->PolicyItem */
    for (Edge<SecurityClasses> edge : pol.getEdges()) {
      Policyitem policyitem = new Policyitem();
      allowed.getPolicyitem().add(policyitem);
      policyitem.setSecurityclass(edge.getFrom().toString());

      Inferences inferences = new Inferences();
      policyitem.setInferences(inferences);

      Securityclasses infsecurityclasses = new Securityclasses();
      inferences.getSecurityclasses().add(infsecurityclasses);

      for (SecurityClasses to : edge.getTo()) {
        infsecurityclasses.getSecurityclass().add(to.toString());
      }
    }

    /* Case->Info->Analysis->Attributes->Attribute->Policy->NotAllowed */
    Notallowed notallowed = new Notallowed();
    policy.setNotallowed(notallowed);

    SortedSet<Edge<SecurityClasses>> nset = null;
    if (precision instanceof AllTrackingPrecision) {
      nset = ((AllTrackingPrecision) precision).getNset();
    }
    if (precision instanceof ImplicitDependencyPrecision) {
      nset = ((ImplicitDependencyPrecision) precision).getNset();
    }

    /* Case->Info->Analysis->Attributes->Attribute->Policy->NotAllowed->PolicyItem */
    if (nset != null) {
      for (Edge<SecurityClasses> edge : nset) {
        Policyitem policyitem = new Policyitem();
        notallowed.getPolicyitem().add(policyitem);
        policyitem.setSecurityclass(edge.getFrom().toString());
    ...
    /* Case->Info->Analysis->Attributes->Attribute->Mapping */
    Mapping mapping = new Mapping();
    attribute.setMapping(mapping);

    Map<Variable, SecurityClasses> scmap = null;
    SecurityClasses defsc = null;
    if (precision instanceof AllTrackingPrecision) {
      scmap = ((AllTrackingPrecision) precision).getScmap();
      defsc = ((AllTrackingPrecision) precision).getDefaultSC();
    }
    if (precision instanceof ImplicitDependencyPrecision) {
      scmap = ((ImplicitDependencyPrecision) precision).getScmap();
      defsc = ((ImplicitDependencyPrecision) precision).getDefaultSC();
    }

    /* Case->Info->Analysis->Attributes->Attribute->Mapping->MapItem */
    for (Entry<Variable, SecurityClasses> entry : scmap.entrySet()) {
      Mapitem mapitem = new Mapitem();
      mapping.getMapitem().add(mapitem);

      mapitem.setEntity(entry.getKey().toString());
      mapitem.setSecurityclass(entry.getValue().toString());
    }
    Mapitem mapitem = new Mapitem();
    mapping.getMapitem().add(mapitem);

    mapitem.setEntity("__default");
    mapitem.setSecurityclass(defsc.toString());


    /* Case->Results */
    List<AbstractState> targets =
        FluentIterable.from(pReached).filter(AbstractStates.IS_TARGET_STATE).toList();
    boolean analysisresult = (targets.size() == 0);

    Results results = new Results();
    testcase.setResults(results);

    /* Case->Results->Result */
    ResultExtension result = new ResultExtension();
    results.getResult().add(result);

    /* TODO RESULT TRUE/FALSE
    result.

    /* TODO */

    /* Case->Additional */
    AdditionalExtension additional = new AdditionalExtension();
    testcase.setAdditional(additional);
  ...
  public void exportToJSon(UnmodifiableReachedSet pReached) {
    List<AbstractState> targets =
        FluentIterable.from(pReached).filter(AbstractStates.IS_TARGET_STATE).toList();
    JsonObj topContainerR = new JsonObj();
    /* 1. File-Infos */
    JsonObj fileInfoR = new JsonObj();

    /* 1.1. File-Name */
    List<Path> fileName = cfa.getFileNames();
    StringValue fileNameR = new StringValue(fileName.get(0).getFileName().toString());

    fileInfoR.add(new StringValue("File-Name"), fileNameR);

    /* 1.2. Entities */
    JsonArray fileEntitiesR = new JsonArray();

    VariableClassification vc = cfa.getVarClassification().get();

    Set<String> vars = new TreeSet<>();
    vars.addAll(vc.getIntBoolVars());
    vars.addAll(vc.getIntAddVars());
    vars.addAll(vc.getIntEqualVars());

    for (String var : vars) {
      fileEntitiesR.add(new StringValue(var));
    }

    fileInfoR.add(new StringValue("File-Entities"), fileEntitiesR);

    topContainerR.add(new StringValue("File-Info"), fileInfoR);

    /* 2. Main-Analysis */
    mainAnalysisToJson(pReached, topContainerR, vars);
