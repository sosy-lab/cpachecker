...
public class CallstackTransferRelation extends SingleEdgeTransferRelation {
  ...
  private static final CFANode UNDEFINED_CALL_NODE = CFANode.newDummyCFANode("__undefined__");
      ...
      case FunctionCallEdge:
        {
          ...
          assert calledFunction.equals(state.getCurrentFunction()) || isWildcardState(state, AnalysisDirection.BACKWARD);

          if (isWildcardState(state, AnalysisDirection.BACKWARD)) {
  ...
  protected boolean skipRecursiveFunctionCall(final FunctionReturnEdge callEdge) {
    // Cannot skip if there is no edge for skipping
    // (this would just terminate the path here -> unsound).
    if (enteringEdges(callEdge.getPredecessor())
