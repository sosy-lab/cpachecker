...
public class RelyGuaranteeRefinementManager<T1, T2> extends PredicateRefinementManager<T1, T2>  {
  ...
  public List<InterpolationDag> getDags(ReachedSet[] reachedSets, ARTElement errorElem, int tid) throws InterruptedException, CPAException {
    ...
    if (debug){
     ...
     ListMultimap<InterpolationDagNode, RelyGuaranteeCFAEdge> appliedMap = mainDag.getAppliedEnvEdges(tid);
    ...
    if (branches.isEmpty()){
      ...
      */
      Pair<Integer, Integer> lastNodeKey  = branch.get(branch.size()-1);
      InterpolationDagNode lastNode       = dag.getNode(lastNodeKey);
      if (lastNode == null){
        System.out.println();
      }
      ...
      for (InterpolationDagNode appNode : appliedMap.keySet()){
        if (!errorPath.contains(Pair.of(appNode.getTid(), appNode.getArtElement().getElementId()))){
          continue;
        }
        ...
        if (!allPf.getFormula().isTrue()){
          InterpolationDagNode dagAppNode = dag.getNode(appNode.getTid(), appNode.getArtElement().getElementId());
          if (dagAppNode == null){
            System.out.println();
          }
          allPf = pmgr.makeAnd(dagAppNode.getPathFormula(), allPf);
          dag.replacePathFormulaInNode(dagAppNode, allPf);
        }
      }
  ...
  public void getRootsForElement(ReachedSet[] reachedSets, ARTElement errorElem, int tid, InterpolationDag dag, IntegerWrapper newUnique) throws InterruptedException, CPAException {
