...
public class RelyGuaranteeEnvironment {
  ...
  private final Vector<RelyGuaranteeEnvironmentalTransition>[] envTransitionsCreatedBy;
  ...
  public RelyGuaranteeEnvironment(int threadNo, Configuration config, LogManager logger){
    ...
    envTransitionsCreatedBy = new Vector[threadNo];
  ...
  public void removeUnappliedEnvEdgesForThread(int i) {
  ...
  /**
   * Convert environmental transitions into relevant CFA edges.
  ...
  public void processEnvTransitions(int i) {
    ...
    while (!rgEdges.isEmpty()){
      ...
      for (int j=0; j<this.threadNo; j++){
        if (j!=i) {
          this.addNewEnvEgde(j, new RelyGuaranteeCFAEdge(edge));
        }
      }
    }
  }
