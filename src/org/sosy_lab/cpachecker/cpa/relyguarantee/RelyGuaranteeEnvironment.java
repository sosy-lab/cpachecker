...
public class RelyGuaranteeEnvironment {
  ...
  public void processEnvTransitions(int i) {
    ...
    RelyGuaranteeCFAEdgeTemplate edge;
    while (!rgEdges.isEmpty()){
      edge = rgEdges.remove(0);
      System.out.println(edge);
      validEnvEdgesFromThread[i].add(edge);
  ...
  private void syntacticCoverageCheck(int i) {
    ...
    Multimap<ARTElement, RelyGuaranteeEnvironmentalTransition> producedBefore = envTransProcessedBeforeFromThread[i];
    ...
    for (ARTElement key : unprocessedTransitions.keys()){
      for (RelyGuaranteeEnvironmentalTransition  et: unprocessedTransitions.get(key)){
        ...
        else if (producedBefore.get(key).contains(et)){
          toDelete.add(Pair.of(key, et));
   ...
   */
  private void semanticCoverageCheck(Vector<RelyGuaranteeCFAEdgeTemplate> rgEdges, int i) {

    //Vector<RelyGuaranteeCFAEdge> uncovered = new Vector<RelyGuaranteeCFAEdge>(rgEdges);
    Vector<RelyGuaranteeCFAEdgeTemplate> toProcess = new Vector<RelyGuaranteeCFAEdgeTemplate>(rgEdges);
   ...
   while(! toProcess.isEmpty()){
     ...
     // TODO fix that
     List<RelyGuaranteeCFAEdgeTemplate> envEdgesForThread = validEnvEdgesFromThread[i];
     for (RelyGuaranteeCFAEdgeTemplate oldEdge : envEdgesForThread){
