...
public class RGEnvironmentManager {
  ...
  private final Vector<RGEnvironmentalTransition> unprocessedTransitions;
  ...
  private final Set<RGCFAEdgeTemplate>[] coveredEnvEdgesFromThread;
  ...
  public RGEnvironmentManager(int threadNo, RGVariables vars, Configuration config, LogManager logger) throws InvalidConfigurationException{
    ...
    this.unprocessedTransitions = new Vector<RGEnvironmentalTransition>();
    ...
    for (int i=0; i< threadNo; i++){
      ...
      this.coveredEnvEdgesFromThread[i] = new HashSet<RGCFAEdgeTemplate>();
...
/*
   ...
   */
  public void addEnvTransitions(Vector<RGEnvironmentalTransition> newTransitions) {
    if (!newTransitions.isEmpty()){
      unprocessedTransitions.addAll(newTransitions);
    }
  }

  public void addEnvTransition(RGEnvironmentalTransition newTransition) {
  ...
  public List<RGEnvironmentalTransition> getUnprocessedTransitions(){
  ...
  public void processEnvTransitions(int i) {
    ...
    Vector<RGCFAEdgeTemplate> rgEdges = new Vector<RGCFAEdgeTemplate>();
    for (RGEnvironmentalTransition  et: unprocessedTransitions){
     ...
     /* PathFormula opPf = null;
      ...
      RGCFAEdgeTemplate rgEdge = generateEnvTransition(et);
      rgEdges.add(rgEdge);
    }
    ...
    if (checkEnvTransitionCoverage) {
      ...
      Pair<Vector<RGCFAEdgeTemplate>, Vector<RGCFAEdgeTemplate>> pair = semanticCoverageCheck(rgEdges, i);
   ...
   * @return
   */
  private RGCFAEdgeTemplate generateEnvTransition(RGEnvironmentalTransition et) {
    ...
    if (this.abstractEnvTransitions == 2){
      ...
      return new RGAbstractCFAEdgeTemplate(newAbs, lastARTAbstractionElement, et);
    }
      ...
      return new RGAbstractCFAEdgeTemplate(aFilter, lastARTAbstractionElement, et);
    } else {
      ...
      return new RGCFAEdgeTemplate(filter, lastARTAbstractionElement, et);
    }
  }
  ...
  private void assertion(){
    for (int i=0; i<threadNo; i++){
      // valid
      for (RGCFAEdgeTemplate et : validEnvEdgesFromThread[i]){
        ...
        if (checkEnvTransitionCoverage){
          for (RGCFAEdgeTemplate et2 : validEnvEdgesFromThread[i]){
      ...
      // covered
      for (RGCFAEdgeTemplate et3 : coveredEnvEdgesFromThread[i]){
    ...
    // unapplied
    for (int j=0; j<threadNo; j++){
      for (RGCFAEdgeTemplate et: this.unappliedEnvEdgesForThread[j]){
    ...
    // unprocessed
    for ( RGEnvironmentalTransition et: this.unprocessedTransitions){
      assert !et.getSourceARTElement().isDestroyed();
    }
   ...
   * @param string
   */
  private void printTransitions(String string, Collection<RGEnvironmentalTransition> transitions) {
    System.out.println();
    ...
    for (RGEnvironmentalTransition tran : transitions){
  ...
  private void syntacticCoverageCheck(int i) {
    Vector<RGEnvironmentalTransition> toDelete = new Vector<RGEnvironmentalTransition>();
    Vector<RGEnvironmentalTransition> toPrint = new Vector<RGEnvironmentalTransition>();
   ...
   */
  private Pair<Vector<RGCFAEdgeTemplate>, Vector<RGCFAEdgeTemplate>> semanticCoverageCheck(List<RGCFAEdgeTemplate> newEdges, int i) {

    Vector<RGCFAEdgeTemplate> toProcess = new Vector<RGCFAEdgeTemplate>(newEdges);
    Vector<RGCFAEdgeTemplate> toDelete = new Vector<RGCFAEdgeTemplate>();
    ...
    for (RGCFAEdgeTemplate edge1 : toProcess){
      if (!toDelete.contains(edge1)){
        for (RGCFAEdgeTemplate edge2 : toProcess){
    ...
    Vector<RGCFAEdgeTemplate> list1 = new Vector<RGCFAEdgeTemplate>(toProcess);
    Vector<RGCFAEdgeTemplate> list2 = new Vector<RGCFAEdgeTemplate>();

    // sanity check on request
    if (debug){
      for (RGCFAEdgeTemplate edge1 : newEdges){
        for (RGCFAEdgeTemplate edge2 : toProcess){
    ...
    while (!toProcess.isEmpty()){
      RGCFAEdgeTemplate newEdge = toProcess.remove(0);
    ...
    // sanity check on request
    if (debug){
      for (RGCFAEdgeTemplate edge1  : list1){
        for (RGCFAEdgeTemplate edge2  : newEdges){
      ...
      for (RGCFAEdgeTemplate edge1  : list2){
        boolean covered = false;
        assert validEnvEdgesFromThread[i].contains(edge1);
        for (RGCFAEdgeTemplate edge2  : list1){
  ...
  public void mergeSourceElements(ARTElement mergedElement, ARTElement reachedElement, int i) {
    ...
    // covered
    for(RGCFAEdgeTemplate rgEdge : coveredEnvEdgesFromThread[i]){
      ARTElement sourceARTElement = rgEdge.getSourceARTElement();
      if (sourceARTElement == reachedElement){
        rgEdge.setSourceARTElement(mergedElement);
        System.out.println("! Replaced id:"+reachedElement.getElementId()+" by id:"+mergedElement.getElementId()+" in a valid edge: "+rgEdge);
   ...
   * @param tid
   */
  public void killEnvironmetalEdges(Iterable<Integer> tids, ARTReachedSet[] artReachedSets, Multimap<Integer, Pair<ARTElement, RGPrecision>> refinementResult) {
    System.out.println();
  ...
  private void killCovered(int tid) {
    Vector<RGCFAEdgeTemplate> toDelete = new Vector<RGCFAEdgeTemplate>();
    for (RGCFAEdgeTemplate rgEdge : coveredEnvEdgesFromThread[tid]){
  ...
  private void killValid(int tid) {
    Vector<RGCFAEdgeTemplate> toDelete = new Vector<RGCFAEdgeTemplate>();
    for (RGCFAEdgeTemplate rgEdge : validEnvEdgesFromThread[tid]){
      if (rgEdge.getSourceARTElement().isDestroyed()){
        // rgEdge belongs to a dropped subtree
        toDelete.add(rgEdge);
        //makeRelyGuaranteeEnvEdgeFalse(rgEdge);
    ...
    for (RGCFAEdgeTemplate rgEdge : toDelete){
      List<RGCFAEdgeTemplate> covered = new Vector<RGCFAEdgeTemplate>(rgEdge.getCovers());

      rgEdge.killValidEdge();

      for (RGCFAEdgeTemplate edge : covered){
        assert edge.getCoveredBy() == null;
      }

      Pair<Vector<RGCFAEdgeTemplate>, Vector<RGCFAEdgeTemplate>> pair = semanticCoverageCheck(covered, tid);
  ...
  private void killUnapplied() {
    Vector<RGCFAEdgeTemplate> toDelete = new Vector<RGCFAEdgeTemplate>();
    for(int j=0; j<threadNo; j++){
      for (RGCFAEdgeTemplate rgEdge : unappliedEnvEdgesForThread[j]){
        if (rgEdge.getSourceARTElement().isDestroyed()){
  ...
  private Set<Integer> dropApplications(ARTReachedSet[] artReachedSets, Multimap<Integer, Pair<ARTElement, RGPrecision>> refinementResult) {
    ...
    for (Integer tid : toDrop.keySet()){
      ...
      Collection<Pair<ARTElement, RGPrecision>> precisions = refinementResult.get(tid);
      assert precisions.size() <= 1;
      SetMultimap<CFANode, AbstractionPredicate> oldPreds = null;
      for (Pair<ARTElement, RGPrecision> pair : precisions){
      ...
      for (ARTElement artElement : toDrop.get(tid)){
        if (!artElement.isDestroyed()){
          ...
          RGPrecision newRgPrec = new RGPrecision(pmapBuilder.build(), rgPrec.getGlobalPredicates());
...
/*
  ...
  public void removeUnprocessedTransitionsFromElement() {
    ...
    List<RGEnvironmentalTransition> toDelete = new Vector<RGEnvironmentalTransition>();
