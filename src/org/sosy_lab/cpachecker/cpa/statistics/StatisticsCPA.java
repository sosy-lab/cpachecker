...
public class StatisticsCPA extends AbstractCPA implements StatisticsProvider {
  ...
  @Option(
      secure = true,
      name = "metric.floatVariablesCount",
      description =
          "count the number of traversed variable definitions with floating type (float or"
              + " double).")
  private boolean floatVariablesCount = true;

  @Option(
      secure = true,
      name = "metric.dereferenceCount",
  ...
  @Option(
      ...
      name = "analysis",
      description =
          "set this to true when you only want to do a code analysis. If StatisticsCPA is combined"
              + " with other CPAs to do queries use false.")
  ...
  @Option(
      ...
      values = {"sep", "join"},
      description =
          "which merge operator to use for StatisticsCPA? Ignored when analysis is set to true")
  ...
  public StatisticsCPA(Configuration config, LogManager pLogger, CFA cfa)
