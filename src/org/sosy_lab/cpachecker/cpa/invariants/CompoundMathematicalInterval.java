...
public class CompoundMathematicalInterval implements CompoundIntegralInterval {
  ...
  private CompoundMathematicalInterval(SimpleInterval pInterval) {
    intervals = new SimpleInterval[] {pInterval};
  }
  ...
  public List<CompoundMathematicalInterval> splitIntoIntervals() {
    return Lists.transform(Arrays.asList(intervals), CompoundMathematicalInterval::of);
  }
  ...
  public CompoundMathematicalInterval unionWith(SimpleInterval pOther) {
    ...
    if (pOther.hasLowerBound() && hasUpperBound()) {
      ...
      SimpleInterval currentLocal = intervals[start];
    ...
    for (int index = start; index < intervals.length; ++index) {
      SimpleInterval interval = intervals[index];
  ...
  public boolean contains(SimpleInterval pInterval) {
    ...
    while (leftInclusive < rightExclusive) {
      ...
      SimpleInterval intervalAtIndex = intervals[index];
  ...
  public boolean isTop() {
    return !isBottom() && intervals[0].isTop();
  }
  ...
  public String toString() {
    ...
    sb.append('{');
    if (!isBottom()) {
      Iterator<SimpleInterval> intervalIterator = Arrays.asList(intervals).iterator();
  ...
  public boolean hasLowerBound() {
    ...
    return intervals[0].hasLowerBound();
  }
  ...
  public boolean hasUpperBound() {
    ...
    return intervals[intervals.length - 1].hasUpperBound();
  }
  ...
  public boolean equals(Object pOther) {
    ...
    if (pOther instanceof CompoundMathematicalInterval) {
      return Arrays.equals(intervals, ((CompoundMathematicalInterval) pOther).intervals);
    }
    return false;
  }
  ...
  public CompoundMathematicalInterval invert() {
    ...
    if (!hasLowerBound()) {
      currentLowerBound = intervals[index++].getUpperBound().add(BigInteger.ONE);
    }
    while (index < intervals.length) {
      SimpleInterval current = intervals[index++];
      result =
          result.unionWith(
              createSimpleInterval(
                  currentLowerBound, current.getLowerBound().subtract(BigInteger.ONE)));
  ...
  public CompoundMathematicalInterval negate() {
    ...
    CompoundMathematicalInterval result = bottom();
    for (SimpleInterval simpleInterval : intervals) {
   ...
   * negation, see {@link #negate()}. For state inversion, see {@link #invert()}.
   *
  ...
  public CompoundMathematicalInterval binaryAnd(CompoundMathematicalInterval pState) {
    ...
    if (pState.isSingleton()) {
      ...
      for (SimpleInterval interval : intervals) {
        ...
        result =
            result.unionWith(
                SimpleInterval.singleton(interval.getLowerBound().and(pState.getValue())));
      }
    } else if (isSingleton()) {
  ...
  public CompoundMathematicalInterval absolute() {
    ...
    return intersectWith(one().negate().extendToMinValue())
        .negate()
  ...
  public CompoundMathematicalInterval binaryXor(CompoundMathematicalInterval pState) {
    ...
    if (pState.isSingleton()) {
      ...
      for (SimpleInterval interval : intervals) {
        ...
        result =
            result.unionWith(
                SimpleInterval.singleton(interval.getLowerBound().xor(pState.getValue())));
      }
      return result;
    } else if (isSingleton()) {
  ...
  public CompoundMathematicalInterval binaryOr(CompoundMathematicalInterval pState) {
    ...
    if (pState.isSingleton()) {
      ...
      for (SimpleInterval interval : intervals) {
        ...
        result =
            result.unionWith(
                SimpleInterval.singleton(interval.getLowerBound().or(pState.getValue())));
      }
      return result;
    } else if (isSingleton()) {
  ...
  public static CompoundMathematicalInterval logicalAnd(
      CompoundMathematicalInterval p1, CompoundMathematicalInterval p2) {
  ...
  private <T> CompoundMathematicalInterval applyOperationToAllAndUnite(
      Operator<SimpleInterval, T, CompoundMathematicalInterval> pOperator, T pOperand) {
    CompoundMathematicalInterval result = bottom();
    for (SimpleInterval interval : intervals) {
  ...
  /**
   * Union two simple intervals. The intervals must touch each other for this operation to be
   * possible.
   *
   ...
   * @param first the reference value.
   * @param second the value the reference value is compared to.
   * @return <code>true</code> if <code>a</code> is less than or equal to <code>b</code>, <code>
  ...
  /**
   * Creates a simple interval from the two given bounds. <code>null</code> parameters are allowed
   * and used to represent negative or positive infinity.
   *
  ...
  private static SimpleInterval createSimpleInterval(
      @Nullable BigInteger lowerBound, @Nullable BigInteger upperBound) {
    if (lowerBound == null) {
      if (upperBound == null) {
  ...
  public static CompoundMathematicalInterval span(
      CompoundMathematicalInterval pLeftValue, CompoundMathematicalInterval pRightValue) {
