 ...
 * be applied to an interval operand and a big integer operand,
 * producing another interval representing the result of the operation.
 */
public enum ISIOperator implements Operator<BitVectorInterval, BigInteger, BitVectorInterval> {
  ...
  MULTIPLY {
    ...
    public BitVectorInterval apply(BitVectorInterval pFirstOperand, BigInteger pSecondOperand) {
      ...
      if (pSecondOperand.equals(BigInteger.ZERO)) {
        return BitVectorInterval.cast(pFirstOperand.getBitVectorInfo(), BigInteger.ZERO);
      }
      /*
  ...
  MODULO {
    ...
    public BitVectorInterval apply(BitVectorInterval pFirstOperand, BigInteger pSecondOperand) {
      ...
      if (pFirstOperand.isSingleton()) {
        return BitVectorInterval.cast(pFirstOperand.getBitVectorInfo(), pFirstOperand.getLowerBound().remainder(pSecondOperand));
      }
      ...
      if (pFirstOperand.containsNegative()) {
         ...
         */
        moduloRange = BitVectorInterval.cast(pFirstOperand.getBitVectorInfo(), largestPossibleValue.negate(), BigInteger.ZERO);
        /*
      ...
      if (pFirstOperand.containsPositive()) {
         ...
         * from (divisor-1) to zero
         */
        BitVectorInterval posRange = BitVectorInterval.cast(pFirstOperand.getBitVectorInfo(), BigInteger.ZERO, largestPossibleValue);
        /*
        ...
        if (posPartLength.compareTo(pSecondOperand) < 0) {
          ...
          if (modBorder.compareTo(posPart.getLowerBound()) <= 0
            ...
            posRange = BitVectorInterval.cast(pFirstOperand.getBitVectorInfo(), bound1.min(bound2), bound1.max(bound2));
          }
        }
        /*
   ...
   */
  @Override
  public abstract BitVectorInterval apply(BitVectorInterval pFirstOperand, BigInteger pSecondOperand);
