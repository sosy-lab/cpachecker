...
/**
 * The instance of this class is an operator for right shifting a simple
 * interval by a simple interval to produce another simple interval.
 */
enum ShiftRightOperator implements IIIOperator {
  ...
  public SimpleInterval apply(SimpleInterval pFirstOperand, SimpleInterval pSecondOperand) {
    ...
    if (pSecondOperand.containsNegative()) {
      ...
      SimpleInterval negPartResult = IIIOperator.SHIFT_LEFT_OPERATOR.apply(pFirstOperand, negPart.negate());
    ...
    if (pSecondOperand.containsPositive()) {
       ...
       */
      SimpleInterval posPartResult = ISIOperator.SHIFT_RIGHT_OPERATOR.apply(pFirstOperand, posPart.getLowerBound());
      if (posPart.hasUpperBound()) {
        posPartResult = SimpleInterval.span(posPartResult, ISIOperator.SHIFT_RIGHT_OPERATOR.apply(pFirstOperand, posPart.getUpperBound()));
      } else {
