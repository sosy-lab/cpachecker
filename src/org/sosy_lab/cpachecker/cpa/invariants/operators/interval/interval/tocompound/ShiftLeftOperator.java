...
/**
 * This class represents the left shift operator for left shifting
 * intervals by intervals.
 */
enum ShiftLeftOperator implements IICOperator {
  ...
  public CompoundInterval apply(SimpleInterval pFirstOperand, SimpleInterval pSecondOperand) {
    ...
    if (pSecondOperand.containsNegative()) {
      ...
      result = result.unionWith(ShiftRightOperator.INSTANCE.apply(pFirstOperand, negPart.negate()));
    }
    /*
    ...
    if (pSecondOperand.containsPositive()) {
       ...
       */
      CompoundInterval posPartResult = ISCOperator.SHIFT_LEFT_OPERATOR.apply(pFirstOperand, posPart.getLowerBound());
      if (posPart.hasUpperBound()) {
        posPartResult = CompoundInterval.span(posPartResult, ISCOperator.SHIFT_LEFT_OPERATOR.apply(pFirstOperand, posPart.getUpperBound()));
      } else {
