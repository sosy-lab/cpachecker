...
enum ShiftLeftOperator implements IICOperator {
  ...
  public CompoundState apply(SimpleInterval pFirstOperand, SimpleInterval pSecondOperand) {
    ...
    if (pSecondOperand.containsPositive()) {
       ...
       */
      CompoundState posPartResult = ISCOperator.SHIFT_LEFT_OPERATOR.apply(pFirstOperand, posPart.getLowerBound());
      if (posPart.hasUpperBound()) {
        posPartResult = CompoundState.span(posPartResult, ISCOperator.SHIFT_LEFT_OPERATOR.apply(pFirstOperand, posPart.getUpperBound()));
      } else {
