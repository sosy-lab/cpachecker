...
public class ProductAutomatonStopOperator extends CompositeStopOperator {
  ...
  @Override
  public boolean stop(AbstractState pCompositeState,
      Collection<AbstractState> pCompositeReachedState,
      Precision pCompositePrecision)
    ...
    for (int idx = 0; idx < compositeElements1.size(); idx++) {
      ...
      Precision absPrecision = null;

      if (pCompositePrecision instanceof CompositePrecision) {
        absPrecision = ((CompositePrecision) pCompositePrecision).get(idx);
      }

      if (!stopOp.stop(absElem1, Collections.singleton(absElem2), absPrecision)) { return false; }
    }
