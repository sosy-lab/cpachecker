...
class HappensBeforeUtils {
   ...
   */
  static <Key, Value> Map<Key, List<Value>> extendMapOfLists(final Map<Key, List<Value>> base, final Key key, final Value value) {
    final ImmutableMap.Builder<Key, List<Value>> newMap = ImmutableMap.builder();
    newMap.putAll(base);
    newMap.put(key, ImmutableList.<Value>builder().addAll(base.getOrDefault(key, List.of())).add(value).build());
    return newMap.buildKeepingLast();
  }
   ...
   */
  static <Key, Value> Map<Key, List<Value>> insertIntoMapOfLists(final Map<Key, List<Value>> base, final int idx, final Key key, final Value value) {
    final ImmutableMap.Builder<Key, List<Value>> newMap = ImmutableMap.builder();
    newMap.putAll(base);
    ...
    newMap.put(key, ImmutableList.<Value>builder().addAll(oldList.subList(0, idx)).add(value).addAll(oldList.subList(idx, oldList.size())).build());
    return newMap.buildKeepingLast();
  }
   ...
   */
  static <Key, Value> Map<Key, Set<Value>> subtractFromMapOfSets(final Map<Key, Set<Value>> base, final Key key, final Value value) {
   ...
   * Creates a new Map resolving to a Set, with the specified element remove from a set.
   */
  static <Key, Value> Map<Key, Set<Value>> subtractFromMapOfSets(final Map<Key, Set<Value>> base, final Key key, final Collection<Value> value) {
    if(value.isEmpty()) return base;
    checkArgument(base.containsKey(key) && base.get(key).containsAll(value), "Cannot remove non-existent element");
    final ImmutableMap.Builder<Key, Set<Value>> newMap = ImmutableMap.builder();
    newMap.putAll(base);
    newMap.put(key, Sets.difference(base.getOrDefault(key, Set.of()), ImmutableSet.copyOf(value)).immutableCopy());
    return newMap.buildKeepingLast();
  }
