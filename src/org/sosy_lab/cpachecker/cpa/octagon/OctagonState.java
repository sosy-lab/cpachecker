...
/**
 * An element of octagon abstract domain. This element contains an {@link Octagon} which is the
 * concrete representation of the octagon and a map which provides a mapping from variable names to
 * variables.
 */
...
public class OctagonState implements AbstractState {
  ...
  /**
   * This method forgets some information about previously catched variables, this is necessary for
   * isLessOrEquals, or the union operator, be careful using it in other ways (Variables removed
   * from the OctState cannot be referenced anymore by the OctTransferrelation)
   *
   * @param oct the octagon which has the preferred size, instead of just using an int, the OctState
   *     is the parameter, so we can check if the variables are matching if not an Exception is
   *     thrown
   */
  public Pair<OctagonState, OctagonState> shrinkToFittingSize(OctagonState oct) {
    ...
    if (oct.variableToIndexMap.size() != maxEqualIndex + 1) {
      ...
      Octagon newOct2 =
          octagonManager.removeDimension(
              oct.octagon, oct.variableToIndexMap.size() - (maxEqualIndex + 1));
  ...
  /**
   * This method makes an interval assignment to a variable. The OctCoefficients need to be twice as
   * long as for normal assignments! (upper and lower bound)
   */
  ...
  private OctagonState addConstraint(
      BinaryConstraints cons, int leftIndex, int rightIndex, OctagonNumericValue constantValue) {
  ...
  /**
   * This method adds a smaller constraint between two variables (p.e. a <= b). Note that this only
   * works with integers!
   */
  public OctagonState addSmallerEqConstraint(
      MemoryLocation pLeftVarName, MemoryLocation pRightVarName) {
  ...
  /**
   * This method adds a smaller equal constraint between a variable and a long (p.e. a <= 3). Note
   * that this only works with integers!
   */
  public OctagonState addSmallerEqConstraint(
      MemoryLocation pVariableName, OctagonNumericValue pValueOfLiteral) {
    int varIdx = getVariableIndexFor(pVariableName);
  ...
  public OctagonState addSmallerEqConstraint(
      MemoryLocation pVariableName, IOctagonCoefficients oct) {
    ...
    oct =
        oct.add(
            new OctagonIntervalCoefficients(
                oct.size(), new OctagonInterval(Double.NEGATIVE_INFINITY, 0), this));
  ...
  /**
   * This method adds a smaller constraint between a variable and a long (p.e. a < 3). Note that
  ...
  public OctagonState addSmallerConstraint(
    ...
    return addConstraint(
        BinaryConstraints.PX, varIdx, -1, pValueOfLiteral.subtract(OctagonIntValue.ONE));
  }
  ...
  public OctagonState addSmallerConstraint(MemoryLocation pVariableName, IOctagonCoefficients oct) {
    ...
    oct =
        oct.add(
            new OctagonIntervalCoefficients(
                oct.size(), new OctagonInterval(Double.NEGATIVE_INFINITY, -1), this));
    OctagonState assignedState = makeAssignment(pVariableName, oct);
    return assignedState.intersect(this);
  }

  /**
   * This method adds a greater equal constraint between two variables (p.e. a >= b). Note that this
  ...
  public OctagonState addGreaterEqConstraint(
    ...
    oct =
        oct.add(
            new OctagonIntervalCoefficients(
                oct.size(), new OctagonInterval(0, Double.POSITIVE_INFINITY), this));
    OctagonState assignedState = makeAssignment(pVariableName, oct);
    return assignedState.intersect(this);
  }

  /**
   * This method adds a greater constraint between two variables (p.e. a > b). Note that this only
   * works with integers!
   */
  ...
  /**
   * This method adds a greater constraint between a variable and a literal (p.e. a > 3). Note that
   * this only works with integers!
   */
  public OctagonState addGreaterConstraint(
      MemoryLocation pVariableName, OctagonNumericValue pValueOfLiteral) {
    ...
    // set right index to -1 as it is not used
    return addConstraint(
        BinaryConstraints.MX, varIdx, -1, pValueOfLiteral.add(OctagonIntValue.ONE).mul(-1));
  }
  ...
  public OctagonState addGreaterConstraint(MemoryLocation pVariableName, IOctagonCoefficients oct) {
    ...
    oct =
        oct.add(
            new OctagonIntervalCoefficients(
                oct.size(), new OctagonInterval(1, Double.POSITIVE_INFINITY), this));
    OctagonState assignedState = makeAssignment(pVariableName, oct);
  ...
  /**
   * This method adds an equality constraint between two variables (p.e. a == b). Note that this
   * only works with integers!
   */
  ...
  /**
   * This method adds an equality constraint between a variable and a literal (p.e. a == 3). Note
   * that this only works with integers!
   */
  public OctagonState addEqConstraint(
      MemoryLocation pVariableName, OctagonNumericValue constantValue) {
  ...
  /**
   * This method emulates an inequality constraint for assumptions with two variables. There is no
   * inequality constraint possible in the normal way with the octagon library, as workaraound we
   * added a state for a smaller constraint, and a state for a greater constraint. Note that it only
   * works if both variables are Integers!
   */
  ...
  /**
   * This method emulates an inequality constraint for assumptions with a variable and a long/int.
   * There is no inequality constraint possible in the normal way with the octagon library, as
   * workaraound we added a state for a smaller constraint, and a state for a greater constraint.
