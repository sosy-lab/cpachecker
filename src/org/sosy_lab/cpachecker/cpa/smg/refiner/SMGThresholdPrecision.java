...
public abstract class SMGThresholdPrecision implements Precision {

  private final SMGPrecisionAbstractionOptions options;
  private final BlockOperator blockOperator;
  private final SMGHeapAbstractionThreshold heapAbstractionThreshold;
  private final int threshold = 0;

  public SMGThresholdPrecision(
      SMGPrecisionAbstractionOptions pOptions, BlockOperator pBlockOperator) {
    options = pOptions;
    blockOperator = pBlockOperator;
    heapAbstractionThreshold = new SMGHeapAbstractionThreshold(2, 2, 2);
  }

  public SMGThresholdPrecision(
      SMGPrecisionAbstractionOptions pOptions, BlockOperator pBlockOperator, int pMaxLength) {
    options = pOptions;
    blockOperator = pBlockOperator;
    heapAbstractionThreshold =
        new SMGHeapAbstractionThreshold(pMaxLength + 1, pMaxLength + 1, pMaxLength + 1);
  }

  public static SMGThresholdPrecision createStaticPrecision(
  ...
  public static Precision createStaticPrecision(
      boolean pEnableHeapAbstraction, BlockOperator pBlockOperator, int pMaxLength) {
    SMGPrecisionAbstractionOptions options =
        new SMGPrecisionAbstractionOptions(pEnableHeapAbstraction, false, false);
    return new SMGStaticPrecision(options, pBlockOperator, pMaxLength);
  }

  public abstract Precision withIncrement(Map<CFANode, SMGPrecisionIncrement> pPrecisionIncrement);

  public abstract SMGThresholdPrecision join(SMGThresholdPrecision pPrecision);

  public static SMGThresholdPrecision createRefineablePrecision(SMGThresholdPrecision pPrecision) {
    PersistentMultimap<CFANode, SMGMemoryPath> emptyMemoryPaths = PersistentMultimap.of();
    PersistentMultimap<CFANode, SMGAbstractionBlock> emptyAbstractionBlocks =
  ...
  public SMGHeapAbstractionThreshold getHeapAbstractionThreshold() {
    return heapAbstractionThreshold;
  }

  public abstract Set<SMGAbstractionBlock> getAbstractionBlocks(CFANode location);

  private static class SMGRefineablePrecision extends SMGThresholdPrecision {
    ...
    @Override
    public SMGThresholdPrecision join(SMGThresholdPrecision pPrecision) {
  ...
  private static class SMGStaticPrecision extends SMGThresholdPrecision {
    ...
    public SMGStaticPrecision(
        SMGPrecisionAbstractionOptions pAllowsHeapAbstraction,
        BlockOperator pBlockOperator,
        int pMaxLength) {
      super(pAllowsHeapAbstraction, pBlockOperator, pMaxLength);
    }
