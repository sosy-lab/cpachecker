...
public class SMGMaximumThresholdRefiner extends SMGThresholdRefiner {

  private SMGMaximumThresholdFeasibilityChecker checker;

  private SMGMaximumThresholdRefiner(
      LogManager pLogger,
    ...
    checker =
        new SMGMaximumThresholdFeasibilityChecker(
  ...
  public static final SMGMaximumThresholdRefiner create(ConfigurableProgramAnalysis pCpa)
      throws InvalidConfigurationException {

    ARGCPA argCpa = CPAs.retrieveCPAOrFail(pCpa, ARGCPA.class, SMGMaximumThresholdRefiner.class);
    SMGCPA smgCpa = CPAs.retrieveCPAOrFail(pCpa, SMGCPA.class, SMGMaximumThresholdRefiner.class);
    Set<ControlAutomatonCPA> automatonCpas =
        CPAs.asIterable(pCpa).filter(ControlAutomatonCPA.class).toSet();

    return new SMGMaximumThresholdRefiner(
        smgCpa.getLogger(),
  ...
  protected CounterexampleInfo performRefinementForPaths(
    ...
    if (cex.isSpurious()) {
      refineUsingMaxLength(pArgReached, checker.getCutState(), checker.getMaxLength() + 1);
    }
