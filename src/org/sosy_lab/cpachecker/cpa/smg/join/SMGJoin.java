...
final public class SMGJoin {
  ...
  public SMGJoin(CLangSMG pSMG1, CLangSMG pSMG2) throws SMGInconsistentException {
    ...
    for (Entry<String, SMGRegion> entry : globals_in_smg1.entrySet()) {
      ...
      SMGJoinSubSMGs jss = new SMGJoinSubSMGs(status, opSMG1, opSMG2, smg, mapping1, mapping2, globalInSMG1, globalInSMG2, destinationGlobal, 0, false);
      if (! jss.isDefined()) {
        return;
      }
    ...
    while ( smg1stackIterator.hasNext() && smg2stackIterator.hasNext() ) {
      ...
      for (String localVar : frameInSMG1.getVariables().keySet()) {
        ...
        SMGJoinSubSMGs jss = new SMGJoinSubSMGs(status, opSMG1, opSMG2, smg, mapping1, mapping2, localInSMG1, localInSMG2, destinationLocal, 0, false);
        if (! jss.isDefined()) {
          return;
        }
        status = jss.getStatus();
