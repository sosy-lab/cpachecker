 ...
 */
 public class UsedVariablesCollector {
  /**
   ...
   */
  public Set<String> collectVariables(Path currentErrorPath, Pair<ARGState, CFAEdge> start) {

    determineGlobalVariables(currentErrorPath);

    Set<String> collectedVariables = new HashSet<String>();
    boolean passedStartElement = false;
    for (int i = 0; i < currentErrorPath.size(); i++) {
      if(!passedStartElement) {
        passedStartElement = passedStartElement || currentErrorPath.get(i) == start;
        continue;
      }
  ...
  private void collectVariables(CFAEdge edge, Set<String> collectedVariables) {
    ...
    case FunctionCallEdge:
      ...
      collectVariables(functionCallEdge, func.getFunctionCallExpression(), collectedVariables);
    ...
    case ReturnStatementEdge:
      ...
      if(returnStatementEdge.getExpression() != null)
        collectVariables(returnStatementEdge, returnStatementEdge.getExpression(), collectedVariables);

      collectedVariables.add(edge.getPredecessor().getFunctionName() + "::" + "___cpa_temp_result_var_");
  ...
  private class CollectVariablesVisitor extends DefaultCExpressionVisitor<Void, RuntimeException>
     ...
     */
    public CollectVariablesVisitor(CFAEdge currentEdge, Set<String> collectedVariables) {
      this.currentEdge         = currentEdge;
      this.collectedVariables  = collectedVariables;
    }

    private void collectVariables(String variableName) {
      collectedVariables.add(scoped(variableName, currentEdge.getPredecessor().getFunctionName()));
    }
