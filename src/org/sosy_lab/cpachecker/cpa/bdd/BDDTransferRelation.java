...
public class BDDTransferRelation implements TransferRelation {
  ...
  private BDDState handleFunctionReturnEdge(BDDState state, CFunctionReturnEdge cfaEdge,
    ...
    for (String varName : state.getVars()) {
      newRegion = removePredicate(newRegion, rmgr.createPredicate(varName));
    }
    ...
    Region retVar = rmgr.createPredicate(buildVarName(FUNCTION_RETURN_VARIABLE, false, state.getFunctionName()));
  ...
  private Region makePredicate(CExpression exp, String functionName, BDDPrecision precision) {
    ...
    if (precision.isTracking(scopedVarName)) {
      createdPredicates++;
      return rmgr.createPredicate(scopedVarName);
    } else {
      return null;
    }
  }
