 ...
 * @Description GlobalScope for storing files, copy from sosy_lab.cpachecker.parser.eclipse.c
 * @Author Yinbo Yu
 * @Date 3/6/19 9:14 PM
 * @Version 1.0
 **/
...
public class GlobalScope extends AbstractScope{
    ...
    public void registerDeclaration(CSimpleDeclaration declaration) {
        assert declaration instanceof CVariableDeclaration
                || declaration instanceof CEnumType.CEnumerator
...
//        }
    ...
    private CComplexType createRenamedType(CComplexType oldType) {
        ...
        if (oldType instanceof CCompositeType) {
            ...
            List<CCompositeType.CCompositeTypeMemberDeclaration> newMembers = new ArrayList<>(oldCompositeType.getMembers().size());
            for (CCompositeType.CCompositeTypeMemberDeclaration decl : oldCompositeType.getMembers()) {
                ...
                if (decl.getType() instanceof CPointerType) {
                    newMembers.add(new CCompositeType.CCompositeTypeMemberDeclaration(createPointerField((CPointerType) decl.getType(), oldType,
            ...
            List<CEnumType.CEnumerator> list = new ArrayList<>(((CEnumType) oldType).getEnumerators().size());

            for (CEnumType.CEnumerator c : ((CEnumType) oldType).getEnumerators()) {
                CEnumType.CEnumerator newC = new CEnumType.CEnumerator(c.getFileLocation(), c.getName(), c.getQualifiedName(), c.hasValue() ? c.getValue() : null);
                list.add(newC);
            }
