...
public class CPASecondPassBuilder {
  ...
  private void createCallAndReturnEdges(CFANode node, CFANode successorNode, CFAEdge edge, IASTExpression expr, IASTFunctionCallExpression functionCall) {
    ...
    if(fDefNode == null){
      ...
      callEdge = new FunctionCallEdge(functionCall.getRawSignature(), edge.getLineNumber(), node, edge.getSuccessor(), parameters, true);
      callEdge.addToCFA();
    ...
    callEdge = new FunctionCallEdge(functionCall.getRawSignature(), edge.getLineNumber(), node, fDefNode, parameters, false);
    callEdge.addToCFA();
