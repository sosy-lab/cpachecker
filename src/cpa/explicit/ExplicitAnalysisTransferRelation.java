...
public class ExplicitAnalysisTransferRelation implements TransferRelation {
	...
	public ExplicitAnalysisTransferRelation (ExplicitAnalysisDomain explicitAnalysisfUseDomain)
	{
		...
		// TODO add globalvars at some point
...
//				TODO check type of literal here
					...
					if(truthValue){
						if(expAnalysisElement.contains(getvarName(varName, functionName))){
							if(expAnalysisElement.getValueFor(getvarName(varName, functionName)) != valueOfLiteral){
								expAnalysisElement.update(null);
								//= (ExplicitAnalysisElement)explicitAnalysisDomain.getBottomElement();
							}
						}
						else{
							expAnalysisElement.assignConstant(functionName  + "::" + varName, valueOfLiteral);
						}
					}
					// ! a == 9
					else {
				...
				else if(opType == IASTBinaryExpression.op_notequals)
				{
					if(truthValue){
						if(expAnalysisElement.contains(getvarName(varName, functionName))){
							if(expAnalysisElement.getValueFor(getvarName(varName, functionName)) == valueOfLiteral){
								expAnalysisElement = (ExplicitAnalysisElement)explicitAnalysisDomain.getBottomElement();
							}
						}
						else{
						}
	...
	private void handleDeclaration(ExplicitAnalysisElement expAnalysisElement,
		...
		// TODO we don't need to do anything for declarations
...
//		expAnalysisElement.assignConstant(nameOfVar, value);
//		}
//		else{
//		expAnalysisElement.assignConstant(nameOfVar, value);
//		}
...
//			}
	...
	private void handleVariableAssignment(
		...
		if(expAnalysisElement.contains(rightVarName)){
			expAnalysisElement.assignConstant(leftVarName, expAnalysisElement.getValueFor(rightVarName));
		}
		else{
			expAnalysisElement.forget(leftVarName);
		}
	}
