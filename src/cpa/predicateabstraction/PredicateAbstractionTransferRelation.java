...
public class PredicateAbstractionTransferRelation implements TransferRelation {
  ...
  public PredicateAbstractionTransferRelation(
                                              PredicateAbstractionDomain predAbsDomain) {
  ...
  public AbstractElement getAbstractSuccessor(AbstractElement element,
    ...
    case AssumeEdge: {
      ...
      try {
        handleAssumption(predAbsElement, expression, cfaEdge, assumeEdge
            .getTruthAssumption());
      } catch (PredicateAbstractionTransferException e) {
        e.printStackTrace();
      }
    ...
    case FunctionCallEdge: {
      ...
      // TODO String?
      summaryEdge.registerElementOnSummaryEdge(
          "cpa.predicateabstraction.PredicateAbstractionCPA", predAbsElement);
      ...
      try {
        CPACheckerLogger.log(CustomLogLevel.SpecificCPALevel,
            "Function Call Edge from node "
                + cfaEdge.getPredecessor().getNodeNumber() + " to "
  ...
  public List<AbstractElementWithLocation> getAllAbstractSuccessors(
    ...
    throw new CPAException(
        "Cannot get all abstract successors from non-location domain");
  }
  ...
  private void handleFunctionCall(PredicateAbstractionElement predAbsElement,
                                  ...
                                  CallToReturnEdge summaryEdge)
                                                               throws IOException,
                                                               PredicateAbstractionTransferException {
    FunctionDefinitionNode functionEntryNode =
                                               (FunctionDefinitionNode) functionCallEdge
                                                   .getSuccessor();
    ...
    CPACheckerLogger.log(CustomLogLevel.SpecificCPALevel,
        "Function Paramater: " + functionEntryNode.getFunctionParameterNames());
    ...
    for (int i = 0; i < parameters.size(); i++) {
      ...
      if (parameter.getDeclarator().getPointerOperators().length == 1) {
        IASTPointerOperator pointOp =
                                      parameter.getDeclarator()
                                          .getPointerOperators()[0];
        if (pointOp.getRawSignature().equals("*")) {
          summaryEdge.registerAliasesOnFunctionCalls(argumentName,
              parameterName);
        } else {
          throw new PredicateAbstractionTransferException(
              "Unhandled case --> " + "Only *p operations are handled");
        }
    ...
    instr = instr + " ]";
    CPACheckerLogger.log(CustomLogLevel.SpecificCPALevel,
        "Query for function call: " + instr);

    newElement.updateFunctionCall(previousState, instr);

    CPACheckerLogger.log(CustomLogLevel.SpecificCPALevel,
        "State is updated to: " + newElement);
  ...
  private void handleExitFromFunction(
                                      ...
                                      StatementEdge statementEdge)
                                                                  throws PredicateAbstractionTransferException {

    String functionName = statementEdge.getPredecessor().getFunctionName();
    if (functionName.equals(CPAMain.cpaConfig
        .getProperty("analysis.entryFunction"))) { return; }
    ...
    if (expression instanceof IASTUnaryExpression) {
      ...
      if (unaryExp.getOperator() == IASTUnaryExpression.op_bracketedPrimary) {
        ...
        if (exprInParanhesis instanceof IASTLiteralExpression) {
          IASTLiteralExpression litExpr =
                                          (IASTLiteralExpression) exprInParanhesis;
          ...
          if (typeOfLiteral == IASTLiteralExpression.lk_integer_constant
            ...
            Predicate tempElement =
                                    new Predicate("___cpa_temp_result_var_",
                                        Operator.equals, String.valueOf(val));
          ...
          IASTUnaryExpression unaryExpInParant =
                                                 (IASTUnaryExpression) exprInParanhesis;
          ...
          if (operationInParant == IASTUnaryExpression.op_star) {
            ...
            String modifiedReturnValue =
                                         PredAbstractionConstants
                                             .getStarOperator(variableName);
            Predicate tempElement =
                                    new Predicate("___cpa_temp_result_var_",
                                        Operator.equals, modifiedReturnValue);
            tempElement.setTruthValue(ThreeValuedBoolean.TRUE);
            predAbsElement.addPredicateOnTheFly(tempElement);
          } else {
            throw new PredicateAbstractionTransferException("Unhandled case");
  ...
  private void handleReturnFromFunction(
    ...
    if (summaryEdge.hasAnyPointerParameters()) {
      for (AliasedPointers ap : summaryEdge.getAliasedPointersList()) {
        ...
        Predicate pred =
                         new Predicate(ap.getFirstVar(), Operator.equals, ap
                             .getSecondVar());
    ...
    if (methodCallExpression instanceof IASTFunctionCallExpression) {
      ...
      IASTBinaryExpression binExpr =
                                     (IASTBinaryExpression) methodCallExpression;
      ...
      Predicate pred =
                       new Predicate(modifiedVariableName, Operator.equals,
    ...
    PredicateAbstractionElement newElement =
                                             summaryEdge
                                                 .retrieveAbstractElement("cpa.predicateabstraction.PredicateAbstractionCPA");
    CPACheckerLogger.log(CustomLogLevel.SpecificCPALevel, "New element is "
                                                          + newElement);
    ...
    CPACheckerLogger.log(CustomLogLevel.SpecificCPALevel,
        "Return from function, prev region" + " is " + prevRegion
            + " exit region is " + exitRegion);
  ...
  private void handleStatement(PredicateAbstractionElement predAbsElement,
  ...
  private void handleAssumption(PredicateAbstractionElement predAbsElement,
    ...
    if (expression instanceof IASTBinaryExpression) {
      ...
      if (opType == IASTBinaryExpression.op_greaterThan) {
        ...
        if (truthValue) {
          propagateBooleanExpression(predAbsElement,
              IASTBinaryExpression.op_greaterThan, leftOperator, rightOperator);
        } else {
          propagateBooleanExpression(predAbsElement,
              IASTBinaryExpression.op_lessEqual, leftOperator, rightOperator);
        }
      }
      // a >= 9
        ...
        if (truthValue) {
          propagateBooleanExpression(predAbsElement,
              IASTBinaryExpression.op_greaterEqual, leftOperator, rightOperator);
        } else {
          propagateBooleanExpression(predAbsElement,
              IASTBinaryExpression.op_lessThan, leftOperator, rightOperator);
        }
      }
      // a < 9
      ...
      // a == 9
        ...
        if (truthValue) {
          propagateBooleanExpression(predAbsElement,
              IASTBinaryExpression.op_equals, leftOperator, rightOperator);
        } else {
          propagateBooleanExpression(predAbsElement,
              IASTBinaryExpression.op_notequals, leftOperator, rightOperator);
        }
      }
      // a != 9
    ...
    // Unary operation
        ...
        if (exp1 instanceof IASTUnaryExpression) {
          ...
          if (unaryExp1.getOperator() == IASTUnaryExpression.op_bracketedPrimary) {
            ...
            if (exp2 instanceof IASTBinaryExpression) {
              ...
              throw new PredicateAbstractionTransferException(
                  "Unhandled case " + cfaEdge.getPredecessor().getNodeNumber());
            }
          } else {
            throw new PredicateAbstractionTransferException(
                "Unhandled case " + cfaEdge.getPredecessor().getNodeNumber());
      ...
      if (truthValue) {
        propagateBooleanExpression(predAbsElement,
            IASTBinaryExpression.op_notequals, leftOperator, "0");
      } else {
        propagateBooleanExpression(predAbsElement,
            IASTBinaryExpression.op_equals, leftOperator, "0");
      }
    }
      ...
      String leftOperator =
                            "\"".concat(filedRefExp.getRawSignature()).concat(
                                "\"");
  ...
  private void propagateBooleanExpression(
                                          PredicateAbstractionElement predAbsElement,
                                          int opType, String leftOperator,
                                          String rightOperator)
                                                               throws PredicateAbstractionTransferException {
  ...
  private void handleUnaryExpression(
    ...
    if (operator == IASTUnaryExpression.op_postFixIncr
      ...
      simpIns =
                new SimplifiedInstruction(operandName, "+ [ " + operandName
                                                       + " 1 " + "]",
                    Operator.equals);
    }
    ...
    // If first operand is a variable
      ...
      if (op2 instanceof IASTLiteralExpression) {
        ...
        if (typeOfLiteral == IASTLiteralExpression.lk_integer_constant
          ...
          if (typeOfOperator == IASTBinaryExpression.op_plusAssign) {
            ...
            simpIns =
                      new SimplifiedInstruction(leftVar, " + [ " + leftVar
                                                         + " " + rightVar
                                                         + " ]",
                          Operator.equals);
          }
          ...
          // a *= 2
            ...
            simpIns =
                      new SimplifiedInstruction(leftVar, " * " + rightVar + " "
                                                         + leftVar,
                          Operator.equals);
          }
        } else {
      ...
      // a op= b
        ...
        // a *= b
          ...
          String leftVar = op1.getRawSignature();
          simpIns =
                    new SimplifiedInstruction(leftVar,
                        "__________cpa_________unknownVal___", Operator.equals);
        }
  ...
  private void handleAssignment(PredicateAbstractionElement predAbsElement,
                                IASTBinaryExpression binaryExpression,
                                CFAEdge cfaEdge)
                                                throws PredicateAbstractionTransferException {
        ...
        if (operand instanceof IASTIdExpression) {
          ...
          if (op2 instanceof IASTLiteralExpression
            ...
            SimplifiedInstruction simpIns =
                                            new SimplifiedInstruction(
                                                PredAbstractionConstants
                                                    .getStarOperator(variableName),
                                                rightVariable, Operator.equals);
            handleAssignmetQuery(predAbsElement, simpIns);
          }
        } else {
          throw new PredicateAbstractionTransferException("Unhandled case "
    ...
    // If first operand is a variable
      ...
      if (op2 instanceof IASTLiteralExpression
        ...
        SimplifiedInstruction simpIns =
                                        new SimplifiedInstruction(leftVariable,
                                            rightVariable, Operator.equals);
        handleAssignmetQuery(predAbsElement, simpIns);
      }
      ...
      // a = b op c
        ...
        String leftVariable = op1.getRawSignature();
        handleAssignmentOfBinaryOp(predAbsElement, leftVariable,
            binaryExpression, cfaEdge);
      }
    ...
    // a->s = X
      ...
      if (op2 instanceof IASTLiteralExpression
        ...
        String leftVariable =
                              "\"".concat(fieldRef.getRawSignature()).concat(
                                  "\"");
        String rightVariable = op2.getRawSignature();
        int binaryOp = binaryExpression.getOperator();
  ...
  private void handleAssignmentOfBinaryOp(
                                          PredicateAbstractionElement predAbsElement,
                                          String leftVariable,
                                          IASTBinaryExpression binaryExp,
                                          CFAEdge cfaEdge)
                                                          throws PredicateAbstractionTransferException {
    IASTBinaryExpression binaryExpression =
                                            (IASTBinaryExpression) binaryExp
    ...
    case IASTBinaryExpression.op_plus: {
      ...
      rightVariable =
                      "+ [ " + leftVarNameInBinExp + " " + rightVarNameInBinExp
                          + " ]";
      simpIns =
                new SimplifiedInstruction(leftVariable, rightVariable,
                    Operator.equals);
    ...
    case IASTBinaryExpression.op_multiply: {
      ...
      if (lVarInBinaryExp instanceof IASTUnaryExpression) {
        IASTUnaryExpression unaryExpression =
                                              (IASTUnaryExpression) lVarInBinaryExp;
        int operator = unaryExpression.getOperator();
      ...
      /** a = b * ? */
        ...
        // a = b * -2
          ...
          IASTUnaryExpression unaryExpression =
                                                (IASTUnaryExpression) rVarInBinaryExp;
          int operator = unaryExpression.getOperator();
      ...
      /** a = 8 * ? */
        ...
        int typeOfCastLiteral =
                                ((IASTLiteralExpression) lVarInBinaryExp)
                                    .getKind();
  ...
  private void handleCasting(PredicateAbstractionElement predAbsElement,
                             IASTExpression op1, IASTExpression op2,
                             CFAEdge cfaEdge)
                                             throws PredicateAbstractionTransferException {
    // String functionName = cfaEdge.getPredecessor().getFunctionName();
    String leftVar = op1.getRawSignature();
  ...
  private void handleAssignmetQuery(PredicateAbstractionElement predAbsElement,
                                    SimplifiedInstruction ins) {
    String prevState = predAbsElement.getRegion();
    try {
  ...
  private void handleExternalCalls(PredicateAbstractionElement predAbsElement,
                                   FunctionCallEdge functionCallEdge)
                                                                     throws PredicateAbstractionTransferException {

    IASTExpression expr =
                          functionCallEdge.getSuccessor()
                              .getEnteringSummaryEdge().getExpression();
    ...
    if (expr instanceof IASTBinaryExpression) {
      ...
      if (leftHandSideExp instanceof IASTIdExpression) {
        ...
        SimplifiedInstruction simpIns =
                                        new SimplifiedInstruction(
                                            variableName,
                                            "__________cpa_________unknownVal___",
                                            Operator.equals);
        handleAssignmetQuery(predAbsElement, simpIns);
