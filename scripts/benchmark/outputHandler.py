...
class Statistics:
    ...
    def __init__(self):
        ...
        self.dic[(result.CATEGORY_WRONG, result.STR_TRUE_PROP)] = 0
        self.counter = 0

    def addResult(self, category, status):
        self.counter += 1
        assert category in self.dic
        if category == result.CATEGORY_WRONG and status == result.STR_TRUE_PROP:
            self.dic[(result.CATEGORY_WRONG, result.STR_TRUE_PROP)] += 1
        self.dic[category] += 1
    ...
    def printToTerminal(self):
        Util.printOut('\n'.join(['\nStatistics:' + str(self.counter).rjust(13) + ' Files',
                 ...
                 '    false positives:' + str(self.dic[result.CATEGORY_WRONG] - self.dic[(result.CATEGORY_WRONG, result.STR_TRUE_PROP)]).rjust(4) + \
                 '        (result is false, file is true or has a different false-property)',
                 '    false negatives:' + str(self.dic[(result.CATEGORY_WRONG, result.STR_TRUE_PROP)]).rjust(4) + \
