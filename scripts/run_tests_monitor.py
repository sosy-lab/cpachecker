...
def run_single(benchmark, config, time_limit, mem_limit):
    ...
    retval = p.wait()
    tot_time, outcome, reached, refinements, abstractions, blocksize, maxtime, avtime, maxtimeonpath, maxsizeofpath = None, None, None, None, None, None, None, None, None, None
    ...
    if outcome is None:
        outcome = 'UNKNOWN'
    return tot_time, outcome, reached, refinements, abstractions, blocksize, maxtime, avtime, maxtimeonpath, maxsizeofpath
    ...
    try:
        ...
        for config in map(configname, configs):
            with open(outfile + '.' + config + '.log', 'w') as out:
                ...
                out.write(Template('$i\t$t\t$o\t\tReached\tRefinements\tAbstractions\t\tBlocksize\tMaxTimeForPost\tAvTime\tMaxTimeonPath\tMaxSizeofSinglePath\n').substitute(
                    i='INSTANCE'.ljust(maxlen), t='TIME'.rjust(maxlentime),
                    o='OUTCOME'))
                out.write('-' * (maxlen + maxlentime + 145) + '\n')
                line = Template('$fname\t$ftime\t$foutcome\t$freached\t$frefinements\t$fabstractions\t$fblocksize\t$fmaxtime\t$favtime\t$fmaxtimeonpath\t$fmaxsizeofpath')
		    ...
		    fmaxtimeonpath = (d[name][8] if d[name][8] != None else "").ljust(13)
                    fmaxsizeofpath = d[name][9] if d[name][9] != None else ""
