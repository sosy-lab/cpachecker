...
cd $PATH_GMP \
    && ./configure --with-pic \
    && make \
    && cp .libs/libgmp.so.10.5.0 $PATH_INSTALL/libgmp.so

echo -e "\n\n"

echo "Compiling MPFR..."
cd $PATH_MPFR \
    && ./configure --with-pic --with-gmp=$PATH_GMP/install \
    && make \
    && cp src/.libs/libmpfr.so.6.2.1 $PATH_INSTALL/libmpfr.so
...
cd $PATH_MPFRJAVA \
   ...
   && mvn install -Dmpfr.cppflags="-I${PATH_GMP}/install/include -I${PATH_MPFR}/install/include" -Dmpfr.ldflags="-L${PATH_GMP}/.libs -L${PATH_MPFR}/src/.libs" -DskipTests \
   && cp target/native-build/.libs/libmpfr_java-1.4.so $PATH_INSTALL/libmpfr_java.so \
   && cp target/mpfr_java-1.4.jar $PATH_INSTALL/mpfr_java.jar
