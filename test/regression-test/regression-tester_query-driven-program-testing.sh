...
configurations="basicblock_foo basicblock_main testgoal001_foo testgoal001_main testgoal002_foo"
...
for cfg in $configurations; do
  ...
  for i in `find ../query-driven-program-testing/test/ -name "*.log"` ; do
    name=`echo $i | sed 's#^../query-driven-program-testing/test/##'`
    dir=`dirname $name`
    mkdir -p $LOGDIR/$dir
    mv $i $LOGDIR/$name
  done
  ...
  master="testing/results.master/$cfg.log"
