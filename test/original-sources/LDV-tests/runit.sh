...
for i in `cat $RESDIR/vars_file`; do \
	...
	cd $CPADIR && ./scripts/cpa.sh -config ./test/config/predicateAnalysis.properties -entryfunction main -outputpath $CWD/output-$1.$i.out/ $CWD/$1.$i.cilf.i > $CWD/$1.$i.output 2>&1 || exit 1
	cd $CWD
