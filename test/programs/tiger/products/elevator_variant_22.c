...
# 225 "Elevator.c"
 ...
 int stopRequestedInDirection (int dir, int respectFloorCalls, int respectInLiftCalls) {
  ...
  if (overload && buttonPressed) {
   return stopRequestedInDirection__wrappee__weight(dir, 0, respectInLiftCalls);
  } else return stopRequestedInDirection__wrappee__weight(dir, respectFloorCalls, respectInLiftCalls);
 }
 ...
 int stopRequestedAtCurrentFloor() {
  ...
  } else return stopRequestedAtCurrentFloor__wrappee__weight();
 }
...
void processWaitingOnFloor(int floorID) {
 ...
 void timeShift__wrappee__twothirdsfull () {
 ...
 void printState__wrappee__twothirdsfull () {
# 409 "Elevator.c"
 }
