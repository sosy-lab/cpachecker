...
int main() {
  // Sanity check that we use the correct byte sizes (int pointer in 32 bits is 4 bytes)
  assert(sizeof(int *) == 4);
  ...
  long long llPtr = (long long) (unsigned long) ptr; // double cast to make compiler warning go away
  unsigned long ulPtr = (unsigned long) ptr;

  for (int i = 0; i < memory_size; i++) {
    assert((int *) intPtr == ptr || (int *) uintPtr == ptr || (int *) (unsigned long) llPtr == ptr || (int *) ulPtr == ptr);
    assert(intPtr == (intptr_t) ptr || uintPtr == (uintptr_t) ptr || llPtr == (long long) (unsigned long) ptr || ulPtr == (unsigned long) ptr);
  ...
  for (int i = memory_size - 1; i >= 0; i--) {
    ...
    assert((int *) intPtr == ptr || (int *) uintPtr == ptr ||  (int *) (unsigned long) llPtr == ptr || (int *) ulPtr == ptr);
    assert(*(int *) intPtr == i || *(int *) uintPtr == i || *(int *) (unsigned long) llPtr == i || *(int *) ulPtr == i);
