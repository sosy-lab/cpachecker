...
int main(void) {
  ...
  all_expected_checks_fail = all_expected_checks_fail || (model_saddl_res != (-2147483647L - 1L));
  all_expected_checks_fail = all_expected_checks_fail || (model_saddl_ov != 1);
  ...
  // saddl_min + 1L = saddl_min + 1L, which fits the destination range; stored result = -2147483647L and
  // overflow = 0.
