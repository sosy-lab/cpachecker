...
int main() {
  ...
  oneCorrect = oneCorrect || (useNoArgFunctionPointerGiven(barFunctionPtrNoAddressOp) != 123);

  int (*barFunctionPtrWithAddressOp)();
  barFunctionPtrWithAddressOp = &revBar;
  oneCorrect = oneCorrect || (useNoArgFunctionPointerGiven(barFunctionPtrWithAddressOp) != 321);

  oneCorrect = oneCorrect || (barFunctionPtrNoAddressOp() != 123); // barFunctionPtrNoAddressOp is still working and pointing to the correct function
  oneCorrect = oneCorrect || (barFunctionPtrWithAddressOp() != 321); // barFunctionPtrWithAddressOp is still working and pointing to the correct function
  ...
  oneCorrect = oneCorrect || (useTwoArgFunctionPointerGiven(1, 3, fooFunctionPtrNoAddressOp) != 4);
  oneCorrect = oneCorrect || (useTwoArgFunctionPointerGiven(1, 3, fooFunctionPtrWithAddressOp) != 3);

  oneCorrect = oneCorrect || (fooFunctionPtrNoAddressOp(1, 3) != 4); // fooFunctionPtrNoAddressOp is still working and pointing to the correct function
  oneCorrect = oneCorrect || (fooFunctionPtrWithAddressOp(1, 3) != 3); // fooFunctionPtrWithAddressOp is still working and pointing to the correct function
  ...
  oneCorrect = oneCorrect || (fooFunctionPtrNoAddressOp(1, 3) != 3);
  fooFunctionPtrWithAddressOp = returnFunctionDependingOnInput(1); // fooFunctionPtrWithAddressOp is assigned fooPlus()!
  oneCorrect = oneCorrect || (fooFunctionPtrWithAddressOp(1, 3) != 4);

  oneCorrect = oneCorrect || (useTwoArgFunctionPointerGiven(1, 3, fooFunctionPtrNoAddressOp) != 3);
  oneCorrect = oneCorrect || (useTwoArgFunctionPointerGiven(1, 3, fooFunctionPtrWithAddressOp) != 4);


  // Test creation and execution of function pointers in methods.
  oneCorrect = oneCorrect || (getAndUseTwoArgFunctionPointerGiven(2, 4, 0) != 8); // fooMult(2, 4)
  oneCorrect = oneCorrect || (getAndUseTwoArgFunctionPointerGiven(3, 6, 1) != 9); // fooPlus(3, 6)

  // useTwoArgFunctionPointerGiven(1, 1, fooFunctionPtrNoAddressOp) = 1 -> fooPlus(2,4) = 6
  oneCorrect = oneCorrect || (getAndUseTwoArgFunctionPointerGiven(2, 4, useTwoArgFunctionPointerGiven(1, 1, fooFunctionPtrNoAddressOp)) != 6);

  // useTwoArgFunctionPointerGiven(0, 0, fooFunctionPtrWithAddressOp) = 0 -> fooMult(3,6) = 18
  oneCorrect = oneCorrect || (getAndUseTwoArgFunctionPointerGiven(3, 6, useTwoArgFunctionPointerGiven(0, 0, fooFunctionPtrWithAddressOp)) != 18);

  if (!oneCorrect) {
    goto ERROR; // Only reached if none of the checks is correct
  }

  free(dummyMemory);
  return 0;

  ERROR: // Unsafe for MemSafety/MemCleanup as well due to not freeing the malloced pointer
  return 1;
}
