...
extern u8 __VERIFIER_nondet_u8(void);   // Legacy. Not explicitly listed, but sometimes used. Just a unsigned char for LP64.
extern u16 __VERIFIER_nondet_u16(void); // Legacy. Not explicitly listed, but sometimes used. Just a unsigned short for LP64.
extern u32 __VERIFIER_nondet_u32(void); // Legacy. Just a unsigned int for LP64.
extern u64 __VERIFIER_nondet_u64(void); // Legacy. Not explicitly listed, but sometimes used. Just a unsigned long for LP64.
...
// This tests that an analysis sticks to the correct types of values in LP64
...
int main() {
  ...
  // Longs (in LP64 they are larger than int and ranges from -9223372036854775808 to 9223372036854775807)
  // We need 128 bit integers to compare them (or (unsigned) long long) with something thats larger/smaller
  // You can check for signed overflows in GCC using the flag -ftrapv (the program stops if an overflow is detected)
  // First, some base tests with consts
  long long longMax = 9223372036854775807l;
  long long longlongMax = 9223372036854775807ll;
  assert(longMax == longlongMax);
  __int128 longMaxAsInt128 = 9223372036854775807ll;
  long longMin = -9223372036854775807l - 1; // This does not underflow! The min value for long (and long long) for LP64 can not be expressed as const!
  long long longlongMin = -9223372036854775807ll - 1; // This does not underflow!
  assert(longMin < 0); // Proof that the - 1 does not cause underflow!
  assert(longMin == longlongMin);
  assert((long) (longMin - 1lu) == longMax); // We need to prevent signed overflows!
  assert(longMin == (long) (longMax + 1lu));
  assert((long long) (longlongMin - 1llu) == longlongMax);
  assert(longlongMin == (long long) (longlongMax + 1llu));
  __int128 longMinAsInt128 = -9223372036854775807ll - 1;
  __int128 ulongMaxAsInt128 = 18446744073709551615llu;
  assert(longMin == longMinAsInt128);
  assert(longMax == longMaxAsInt128);
  assert((long) (longMin - 1lu) != longMinAsInt128 - 1);
  assert((long) (longMax + 1lu) != longMaxAsInt128 + 1);
  assert(longMinAsInt128 < longMaxAsInt128);
  assert(longMinAsInt128 < 0);
  assert(ulongMaxAsInt128 > longMaxAsInt128);
  unsigned long ulongMax = 18446744073709551615lu;
  unsigned long long ulonglongMax = 18446744073709551615llu;
  assert(ulongMax == ulonglongMax);
  assert(ulongMax + 1 == 0);
  assert(ulongMax + 1 == 0);
  assert(ulongMax == 0lu - 1);
  assert(ulonglongMax == 0llu - 1);
  // You can't declare consts that are larger than (unsigned) long long
  __int128 longMaxPlusOneAsInt128 = longMaxAsInt128 + 1;
  __int128 longMinMinusOneAsInt128 = longMinAsInt128 - 1;
  __int128 ulongMaxPlusOneAsInt128 = ulongMaxAsInt128 + 1;
  assert(longMaxPlusOneAsInt128 > longMaxAsInt128);
  assert(longMinMinusOneAsInt128 < longMinAsInt128);
  assert(ulongMaxPlusOneAsInt128 > ulongMaxAsInt128);
  long long loeng = 9223372036854775807l;
  unsigned long long uloeng = -9223372036854775807ll - 1;
  unsigned long long uloengMax = 18446744073709551615lu;
  assert(longMaxPlusOneAsInt128 > loeng);
  assert(longMinMinusOneAsInt128 < uloeng);
  assert(ulongMaxPlusOneAsInt128 > uloengMax);


  long nondet_long = __VERIFIER_nondet_long();
  assert(nondet_long != longMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(nondet_long != -2147483649ll); // Your analysis is unsound if this fails
  assert(nondet_long < longMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(nondet_long > -2147483649ll); // Your analysis is unsound if this fails
  
  // Test that correct types are directly enforced, not only after assigning to a variable!
  assert(__VERIFIER_nondet_long() != longMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(__VERIFIER_nondet_long() != -2147483649ll); // Your analysis is unsound if this fails
  assert(__VERIFIER_nondet_long() < longMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(__VERIFIER_nondet_long() > -2147483649ll); // Your analysis is unsound if this fails

  // unsigned long is 0 to 18446744073709551615 in LP64
  unsigned long nondet_ulong = __VERIFIER_nondet_ulong();
  assert(nondet_ulong != ulongMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(nondet_ulong != -1ll); // Your analysis is unsound if this fails
  assert(nondet_ulong < ulongMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(nondet_ulong > -1ll); // Your analysis is unsound if this fails
  
  // Test that correct types are directly enforced, not only after assigning to a variable!
  assert(__VERIFIER_nondet_ulong() != ulongMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  ...
  assert(nondet_longlong != longMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(nondet_longlong != longMinMinusOneAsInt128); // Your analysis is unsound if this fails
  
  // Test that correct types are directly enforced, not only after assigning to a variable!
  assert(__VERIFIER_nondet_longlong() != longMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(__VERIFIER_nondet_longlong() != longMinMinusOneAsInt128); // Your analysis is unsound if this fails

  unsigned long long nondet_ulonglong = __VERIFIER_nondet_ulonglong();
  assert(nondet_ulonglong != ulongMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(nondet_ulonglong != -1); // Your analysis is unsound if this fails
  
  // Test that correct types are directly enforced, not only after assigning to a variable!
  assert(__VERIFIER_nondet_ulonglong() != ulongMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(__VERIFIER_nondet_ulonglong() != -1); // Your analysis is unsound if this fails


  u64 nondet_u64 = __VERIFIER_nondet_u64();
  assert(nondet_u64 != ulongMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(nondet_u64 != -1); // Your analysis is unsound if this fails
  
  // Test that correct types are directly enforced, not only after assigning to a variable!
  assert(__VERIFIER_nondet_u64() != ulongMaxPlusOneAsInt128); // Your analysis is unsound if this fails
  assert(__VERIFIER_nondet_u64() != -1); // Your analysis is unsound if this fails
