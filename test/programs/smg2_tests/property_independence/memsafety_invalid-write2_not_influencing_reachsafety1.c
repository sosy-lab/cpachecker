...
void reach_error() { __assert_fail("0", "memsafety_invalid-write2_not_influencing_reachsafety1.c", 3, "reach_error"); }
...
int main() {
  ...
  *miniArray = a;
  *(miniArray + 1) = b;
  *(miniArray + 2) = 0; // valid-deref violated. ReachSafety can no longer be guaranteed.
  *(miniArray + 1) = *(miniArray + 2); // This is NOT 0! 0 was never written, as it is not our memory!

  __VERIFIER_assert_w_assert(*miniArray == a, miniArray);

  // Has a pseudo fail if you assume that *(miniArray + 1) = 0; due to the invalid-deref not happening. 
  // Should be UNKNOWN!
  __VERIFIER_assert_w_assert(*(miniArray + 1) == b, miniArray);  

  free(miniArray);
  return 0;  // MemTrack and MemCleanup violated
}
