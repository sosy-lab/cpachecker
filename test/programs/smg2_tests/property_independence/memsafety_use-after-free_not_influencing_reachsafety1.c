...
void reach_error() { __assert_fail("0", "memsafety_use-after-free_not_influencing_reachsafety1.c", 3, "reach_error"); }
...
int main() {
  ...
  *(miniArray + 1) = b;
  ...
  free(miniArray);
  __VERIFIER_assert_w_assert(*(miniArray + 1) == b, miniArray);  // ReachSafety should return UNKNOWN here due to use after free
