...
void assertPointerToArraysUsageWithSubscriptAndPointer(long long *longlong_a_from_long, char char_a[], unsigned long long *ulonglong_a) {
  ...
  assert(longlong_a_from_long[0] == 29);
  assert(*(longlong_a_from_long + 1) == 54);
  assert(longlong_a_from_long[2] == 79);
  assert(longlong_a_from_long[3] == 104);
  ...
  assertModPointerToArraysUsageWithSubscriptAndPointer((char (*)[3]) char_a + 1, (long long (*)[3]) (longlong_a_from_long + 2), (unsigned long long (*)[5]) (ulonglong_a - 1));
}


// There will be some warning about the casts of long to long long/unsigned long to long pointers/array pointers, this is intentional!
void assertPointerToArraysUsageWithSubscript(unsigned long long (*ulonglong_a)[4], unsigned long (*long_a_as_ulong)[4], char (*char_a)[4]) {
  ...
  assert((*long_a_as_ulong)[0] == 29);
  assert((*long_a_as_ulong)[1] == 54);
  assert((*long_a_as_ulong)[2] == 79);
  assert((*long_a_as_ulong)[3] == 104);
  ...
  assertPointerToArraysUsageWithSubscriptAndPointer(*long_a_as_ulong, *char_a, *ulonglong_a);
}


// Unsigned long and long/longlong behave the same for long in LP64 (ulong only for positive values that are not too large)
long long * assignArraysNested(unsigned long * ulong_ptr_to_array) {
  ...
  *ulong_ptr_to_array = 32768; // Third element in the array
  return ((long long *) ulong_ptr_to_array) + 1;
}
...
unsigned long * assignArrays(unsigned long long (*ulonglong_ptr_array)[4], long (*long_array)[4], char (*char_array)[4], char * char_ptr) {
  ...
  // long long and long are of the same size in LP64
  long long (*long_array_ptr_from_nested)[1] = (long long (*)[1]) assignArraysNested((*long_array) + 2);
  (*long_array_ptr_from_nested)[0] = 2147483647; // Last element in the array
  (*long_array_ptr_from_nested)[0] = (*long_array_ptr_from_nested)[0] * 2; // This would overflow for 32bit longs

  *(*long_array) = -2147483648;
  long_array++; // Now we incremented the size of the entire array and (*long_array_ptr_from_nested)[0] is out of bounds!
  long_array_ptr_from_nested = (long long (*)[1]) long_array;
  (*(long_array_ptr_from_nested - 3))[0] = -32769; // Assign to second entry in the array 
...
int * assertArraysAfterAssignmentAndChangeSome3(char * char_array_1, long long_1[], unsigned long long (*ulonglong_array_2)[3]) {
  ...
  assert(*(((long *) long_1) - 1) == -2147483648ll);
  return ((long *) long_1 - 2);
}

void assertArraysAfterAssignment4(char (*char_a)[3], long (*long_0)[3], unsigned long long ulonglong_arrayey_neg_1[]) {
  ...
  assert(((*long_0) - 1)[1] == -2147483648);
  assert(**long_0 == -2147483648);
  assert(*(2 + (*long_0) - 1) == -32769);
  assert((*long_0)[2] == 32768);
  assert(*(*(long_0) + 3) == 6442450941ll);
...
void assertArraysAfterAssignment2(unsigned long long * ulonglong_a, int * int_ptr_from_long_array_plus_2, char * char_a) {
  char_a++;
  // Ints are 4 bytes, longs 8 in LP64, so we decrement 2x to get from the starting pointer that was incremented twice to only once
  int_ptr_from_long_array_plus_2--;
  ...
  long * long_1 = (long long *) (assertArraysAfterAssignmentAndChangeSome3(*char_a_ptr, (long (*)[3]) int_ptr_from_long_array_plus_2, ulonglong_a) + 4);
  ...
  assertArraysAfterAssignment4(char_a, long_1, ulonglong_a);
}

// This takes arrays, assertArraysAfterAssignment2 takes pointers, this should not be a problems
void assertArraysAfterAssignment(char char_array[], unsigned long long ull_array_from_long[], unsigned long long ulonglong_array[]) {
  assert(ull_array_from_long[0] == 32768);
  assert(ull_array_from_long[1] == 4294967294);
  ull_array_from_long[1] = ull_array_from_long[1] - 1 - ull_array_from_long[-2];
  assertArraysAfterAssignment2(ulonglong_array, ull_array_from_long, char_array);
}


// Safe for AssertionSafety only if all return the correct result
// This tests that an analysis can handle arrays as pointers and array pointers in methods (local and on the stack) in LP64 (this fails for ILP32).
...
// as well as type re-interpretations (but only in types that are equally large, e.g. long to unsigned long or long long)
...
int main() {
  ...
  long long * ll_ptr_from_long = *long_array_ptr_as_long_array;

  ll_ptr_from_long = assignArrays((unsigned long long (*)[4]) ulonglong_array, (long (*)[4]) ll_ptr_from_long, char_array_as_array_ptr, *char_array_as_array_ptr);

  void * char_array_as_void_ptr = (void *) char_array;

  // int_ptr is the address of element long_array[2] here
  assertArraysAfterAssignment(char_array_as_void_ptr, ll_ptr_from_long, ulonglong_array);
