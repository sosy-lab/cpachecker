...
def run_single(benchmark, config, time_limit, mem_limit, outfile):
    ...
    if "cdaudio_simpl1" in benchmark:
       cmdline = Template('ulimit -t $time_limit -v $mem_limit; '
                       '(scripts/cpa.sh -setprop cpa.monitor.brancheslimit=25 -setprop output.disable=true -config "$config" "$benchmark" > '
       ...
       cmdline = Template('ulimit -t $time_limit -v $mem_limit; '
                       '(scripts/cpa.sh -setprop cpa.monitor.brancheslimit=200 -setprop output.disable=true -config "$config" "$benchmark" > '
                       '"$benchmark.$cn.log" 2>&1)').substitute(locals())
    ...
    retval = p.wait()
    tot_time, outcome, reached, refinements, abstractions, locations = None, None, None, None, None, None
    totloc, assumptionloc = None, None
    if retval == 0:
        outcome = None
    elif retval == 134:
    ...
    with open('%s.%s.log' % (benchmark, configname(config))) as f:
        for line in f:
            if tot_time is None and line.startswith(
                ...
                reached = line[line.find(':')+1:].strip()
            ...
            elif totloc is None and line.startswith('  Number of locations:'):
                totloc = line[line.find(':')+1:].strip()
            elif assumptionloc is None and line.startswith('Number of locations with assumptions:'):
                assumptionloc = line[line.find(':')+1:].strip()
    ...
    if locations is None:
        if totloc is None:
           totloc = 0
        if assumptionloc is None:
           assumptionloc = 0
        locations = int(totloc) - int(assumptionloc)
    return tot_time, outcome, reached, refinements, abstractions, locations
...
def main(which, benchmarks, configs, time_limit, mem_limit, outfile,
    ...
    def go(b, c, t, m, o):
        ...
        with open(outfile + '.' + configname(c) + '.log', mode) as out:
                ...
                if write_header: 
                    out.write(Template('$i\t$t\t$o\t\tReached\tRefinements\tAbstractions\t# Locs\n').substitute(i='INSTANCE'.ljust(maxlen), t='TIME'.rjust(maxlentime), o='OUTCOME'))
                    out.write('-' * (maxlen + maxlentime + 86) + '\n')

                line = Template('$fname\t$ftime\t$foutcome\t$freached\t$frefinements\t$fabstractions\t$flocations')
                for name in sorted(d):
                    ...
                    fabstractions = (d[name][4] if d[name][4] != None else "").ljust(16)
	            flocations = d[name][5]
