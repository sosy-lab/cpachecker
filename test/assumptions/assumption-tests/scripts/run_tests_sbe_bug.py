...
def run_single(benchmark, config, time_limit, mem_limit, outfile):
    ...
    if "transmitter.2.cil.c" in benchmark:
      cmdline = Template('ulimit -t $time_limit -v $mem_limit; '
                       '(scripts/cpa.sh -setprop RepetitionsInPathHeuristics.threshold=3 -setprop output.disable=true -config "$config" "$benchmark" > '
                       '"$benchmark.$cn.log" 2>&1)').substitute(locals())
    elif "transmitter.3.cil.c" in benchmark:
      cmdline = Template('ulimit -t $time_limit -v $mem_limit; '
                       '(scripts/cpa.sh -setprop RepetitionsInPathHeuristics.threshold=4 -setprop output.disable=true -config "$config" "$benchmark" > '
                       '"$benchmark.$cn.log" 2>&1)').substitute(locals())
